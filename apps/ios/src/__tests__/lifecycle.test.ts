...
describe('lifecycle', () => {
  beforeEach(async () => {
    ...
    // Track href assignments via Proxy on window.location
  ...
  describe('setDeepLinkHandler', () => {
    ...
    it('allows clearing handler by passing null', () => {
      const handler = vi.fn();
      lifecycle.setDeepLinkHandler(handler);
      // setDeepLinkHandler accepts null to clear the handler
      lifecycle.setDeepLinkHandler(null!);
  ...
  describe('device ID integration', () => {
    ...
    it('includes device ID in stored session', async () => {
      ...
      await vi.waitFor(() => {
        ...
        // Device ID should be a non-empty string (generated by getOrCreateDeviceId)
  ...
  describe('concurrent deep link handling', () => {
    ...
    it('handles multiple rapid deep links without deduplication', async () => {
      // Note: The lifecycle layer intentionally does NOT deduplicate auth codes.
      // The backend /api/auth/desktop/exchange endpoint is idempotent — replaying
      // the same one-time code returns an error (401) which the client handles
      // gracefully by navigating to the sign-in page. This test verifies the
      // client fires all requests and relies on backend idempotency.
      ...
      // Verify each unique code was sent
      const bodies = mockFetch.mock.calls.map(
        (call) => JSON.parse(call[1]?.body as string).code
      );
