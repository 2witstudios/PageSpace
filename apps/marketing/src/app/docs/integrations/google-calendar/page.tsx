...
**Agents use dedicated calendar tools.** When you @mention an [AI Chat](/docs/page-types/ai-chat) or talk to the global assistant, it has access to read-side tools (\`list_calendar_events\`, \`get_calendar_event\`, \`check_calendar_availability\`) and write-side tools (\`create_calendar_event\`, \`update_calendar_event\`, \`delete_calendar_event\`, \`invite_calendar_attendees\`, \`remove_calendar_attendee\`, \`rsvp_calendar_event\`). These act on the PageSpace event table — anything the agent creates is then pushed up to Google by the sync service, so a meeting it books for you lands on your Google Calendar like any other.
