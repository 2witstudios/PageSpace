...
**Deliveries are idempotent per signed request.** Each signed request (its exact timestamp + signature pair) is processed at most once: re-sending the identical bytes — a network-timeout retry, for example — returns \`200 { ok: true, duplicate: true }\` without posting or firing anything again, and an identical request that arrives while the first is still being processed gets a retryable \`409\` (retry it; you'll then see either the duplicate acknowledgment or a fresh delivery if the first attempt failed). A retry you **re-sign** with a fresh timestamp counts as a new delivery, so keep retrying with the original signed bytes when you want at-most-once behavior, and re-sign only when the previous attempt definitively failed with a retryable error.
