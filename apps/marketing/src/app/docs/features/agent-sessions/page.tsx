...
**Who can use the sandbox.** Sandbox access follows the session's **payer** — the drive's owner, or the session's own owner for a driveless Global Assistant session — not necessarily whoever is using it in the moment. If a drive is owned by a Pro-plan account, every member of that drive can use the sandbox in a session scoped to it, even if their own personal account is on Free. If you're on Free yourself, you still get the full session/chat/panes interface; only the terminal and code-execution affordances are disabled, with a prompt to upgrade.
