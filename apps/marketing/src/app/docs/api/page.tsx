...
const domains = [
  { title: "Authentication", href: "/docs/api/auth", icon: KeyRound, description: "Login, signup, OAuth, session management, MCP tokens" },
  { title: "Pages", href: "/docs/api/pages", icon: FileText, description: "CRUD, hierarchy, permissions, bulk operations, agent config" },
  { title: "Drives", href: "/docs/api/drives", icon: HardDrive, description: "Workspace management, members, roles, trash, search" },
  ...
  { title: "MCP", href: "/docs/api/mcp", icon: Server, description: "Document operations, drive listing for MCP clients" },
  ...
  { title: "Admin", href: "/docs/api/admin", icon: Bell, description: "User management, monitoring, notifications" },
];
  ...
  return (
    <div>
      ...
      <p className="text-lg text-muted-foreground mb-4">
        PageSpace exposes a REST API for all operations. All endpoints require authentication via session cookies or MCP Bearer tokens.
      </p>
      <div className="text-sm text-muted-foreground mb-8 space-y-2">
        ...
        <p><strong>Authentication</strong>: Session cookies (web) or <code className="bg-muted px-1.5 py-0.5 rounded text-xs">Authorization: Bearer mcp_...</code> (MCP)</p>
        <p><strong>Content-Type</strong>: <code className="bg-muted px-1.5 py-0.5 rounded text-xs">application/json</code></p>
      </div>
