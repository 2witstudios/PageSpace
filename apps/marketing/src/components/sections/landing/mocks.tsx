...
export const PAGE_TYPES: PageType[] = [
  { id: "tree", label: "The tree", icon: "tree", parent: "", title: "My Workspace", actions: <span className="iconbtn"><Ico name="updown" size="i14" /></span>, desc: "The whole launch in one place. Nothing lives in another tab, so nothing has to be pasted back in.", Body: TreeMock },
  { id: "document", label: "Document", icon: "fileText", parent: "Product Launch", title: "Q1 Planning", actions: null, desc: "The plan, where the team can find it. Ask the AI to rewrite a section and the edit lands here, not in a chat window you copy from.", Body: DocMock },
  { id: "ai-chat", label: "AI Chat", icon: "bot", parent: "Product Launch", title: "Product AI", actions: <><span className="pill">Anthropic<Ico name="chevD" size="i14" /></span><span className="muted">/</span><span className="pill">Opus 4.6<Ico name="chevD" size="i14" /></span></>, desc: "An agent that works from the folder it lives in. No pasting the spec into a prompt. It reads the page next door and files what it makes.", Body: ChatMock },
  { id: "channel", label: "Channel", icon: "channel", parent: "Product Launch", title: "product-launch", actions: <span className="muted" style={{ fontSize: 13 }}>12 members</span>, desc: "Team chat beside the work, not in another app. @-mention an agent and it answers from the positioning doc two pages away.", Body: ChannelMock },
  { id: "task-list", label: "Task List", icon: "task", parent: "Product Launch", title: "Launch Tasks", actions: <span className="seg"><span className="s"><Ico name="book" size="i14" /></span><span className="s on"><Ico name="rows" size="i14" /></span><span className="s"><Ico name="kanban" size="i14" /></span></span>, desc: "The checklist beside the plan it came from. Assign a task to a person or an agent. Neither needs a separate tool.", Body: TaskMock },
  { id: "sheet", label: "Sheet", icon: "sheet", parent: "Product Launch", title: "Budget", actions: <span className="iconbtn"><Ico name="download" size="i14" /></span>, desc: "The budget where the plan is. Ask the AI what is over plan and it reads the cells. No export, no upload.", Body: SheetMock },
  { id: "canvas", label: "Canvas", icon: "canvas", parent: "Product Launch", title: "Launch Site", actions: <span className="pill pri">Publish</span>, desc: "The launch site, built and published from the same tree as the plan behind it. No site builder to sign in to.", Body: CanvasMock },
  { id: "code", label: "Code", icon: "code", parent: "Product Launch", title: "config.json", actions: null, desc: "Config and scripts beside the docs they belong to, in a real editor. Nothing pasted over from somewhere else.", Body: CodeMock },
  { id: "file", label: "File", icon: "file", parent: "Assets", title: "brief.pdf", actions: <span className="pill on"><Ico name="download" size="i14" />Download</span>, desc: "Drop in the brief and the AI can read it and the team can search it. No connector, no sync, no second copy.", Body: FileMock },
