...
import { buildShellHandlers } from '../shell-handler';
...
function makeAuthSuccess(sessionKey = 'shell:shl-1') {
...
describe('shell task holds (wiring)', () => {
  ...
  function build() {
    return buildShellHandlers({
      sessionMap,
      openShell,
      checkAuth,
      socket,
      persistStreamSessionId,
      createTaskHold: createTaskHold as unknown as Parameters<typeof buildShellHandlers>[0]['createTaskHold'],
    });
  }
  ...
  it('creates the hold immediately on a fresh attached connect', async () => {
    ...
    assert({
      ...
      expected: { sessionKey: 'shell:shl-1', hasSprite: true },
    });
  ...
  it('ticks attached again when a viewer reattaches to the live session', async () => {
    ...
    const handlers2 = buildShellHandlers({
      sessionMap,
      openShell,
      checkAuth,
      socket: socket2,
