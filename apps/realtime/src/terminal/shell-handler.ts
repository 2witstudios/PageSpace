 ...
 * sandbox — the re-keyed successor to `agent-terminal-handler.ts`. The whole
 * `{machineId, projectName?, branchName?, name}` address collapses to
 * **`{shellId}`**: the bridge resolves shell row → session → sandbox, so no
 ...
 * in-memory lookup, and `shell:ready` with the buffered scrollback. No sandbox
 * is resolved, no Sprite is woken and no audit row is written, because
 * `checkAuth` hands the sprite half back as an UNCALLED `resolveSandbox` thunk
 * (see `shell-access.ts`) that only the cold path invokes.
 *
 * `billing` meters this PTY session's active-runtime cost against the session's
 * payer — the agent page's drive owner, or the session owner for a
 * global-assistant session — the same hold/gate/settle seam the predecessor
 * used. Omitted -> unmetered (no hold, no settle).
 *
 ...
 * `ShellCheckAuthResult.resolveSandbox`, and therefore ONLY on the cold
 * (create) path — reattaching to a live in-memory session never resolves it.
 */
export type ShellSandboxResult =
  | {
      ok: true;
      shellId: string;
...
/**
 * The CHEAP half of a connect: a DB-only authorization verdict plus the session
 * key, which derives from the shellId alone. Nothing here has touched — let
 * alone woken — a Sprite, and nothing here has reserved a concurrency slot.
 * That is what lets `onConnect` look up a live in-memory session and reattach
 * to it before any sandbox work happens at all, and what lets the 60s re-auth
 * tick re-check a LIVE session's authorization without competing with that very
 * session for its own slot.
 */
export type ShellCheckAuthResult =
  | {
      ok: true;
      sessionKey: string;
       ...
       * `releaseSlot`.
       */
      resolveSandbox: () => Promise<ShellSandboxResult>;
    }
  | { ok: false; reason: string };

export type ShellCheckAuthFn = (args: { userId: string; shellId: string }) => Promise<ShellCheckAuthResult>;

/** An authorized connect — the `ok` arm of a check-auth result. */
export type ShellAccessGranted = Extract<ShellCheckAuthResult, { ok: true }>;
...
export type ConnectPlan =
  ...
  | { kind: 'reattach'; access: ShellAccessGranted; session: TerminalSession }
  | { kind: 'create'; access: ShellAccessGranted };
...
export function planConnect({
  ...
  accessResult: ShellCheckAuthResult;
 ...
 * object rather than a growing list of leading positionals. Both are optional
 * -> the corresponding effect is skipped.
 */
...
export function planColdTailPersist(
  session: Pick<TerminalSession, 'shellId' | 'scrollback' | 'hasOutput'>,
  endedAt: Date,
): PersistColdTailArgs | undefined {
  if (!session.shellId) return undefined;
  return {
 ...
 * heartbeats) serves a viewer's `shell:connect` and a HEADLESS start driven by
 * agent IO over signed HTTP (`shell-io.ts`). The two differ only in who watches
 * the result, which is `EnsureShellSessionRequest`'s business — everything
 * deciding what a session COSTS and how it is collected lives here, in one
 * place, so the callers cannot drift apart on slot accounting, metering or
 * reaping.
 */
export type ShellSessionDeps = {
   ...
   * answer with its final output rather than nothing. Called from the ONE
   * teardown funnel (`endShellSession`) every teardown path goes through — a
   * failed write must never block cleanup, exactly like billing. Omitted -> no
   * cold tail is ever persisted.
   */
  persistColdTail?: PersistColdTailFn;
  /**
   * Sprites Tasks API hold seam. Called once per COLD create with the resolved
   * Sprite; the controller it returns keeps a self-expiring platform task hold
   * alive while work is in progress (viewer attached OR agent output flowing)
   * and releases it when idle, so a sprite can't cold-pause an agent mid-run —
   * and CAN pause the moment the agent goes quiet. Omitted -> no holds.
   */
...
export type ShellHandlerDeps = ShellSessionDeps & {
  socket: SocketLike;
};
...
export type ShellHandlers = {
  ...
  /** No payload (or no `connectionId` on it) — the real socket disconnected — tears down every connection this socket had open. A payload WITH `connectionId` closes just that one pane, leaving the socket's other connections live. */
  onDisconnect(payload?: unknown): void;
};

/** Extracts the caller-supplied per-pane connection id from an input/resize/disconnect payload, if any — falls back to the socket's own id (this connection's ONLY shell session) for callers that multiplex nothing. */
...
/**
 * Resolve WHAT to actually exec for a fresh session: a per-shell `command`
 ...
 * whitespace; the `'shell'` sentinel (see `shell-types.ts`) resolves to the
 * actual interactive shell binary; any other agentType's resolved command/args
...
function endShellSession(
  ...
  if (deps.persistColdTail && coldTail) {
    void deps.persistColdTail(coldTail).catch((error) => {
      loggers.realtime.error('Failed to persist shell cold tail', error instanceof Error ? error : new Error(String(error)), {
        sessionKey,
      });
    });
  }
 ...
 * session (endShellSession clears all timers), and notify the viewer.
 *
 ...
 * needs a genuine, permanent termination, so it maps to the same `TeardownPlan`
 * an explicit kill would, but the trigger name says plainly who actually
 * decided this — an incident review reading this trigger off a log line should
 * not conclude the user did it.
 */
function teardownShellSession(
  deps: SessionEndDeps,
  sessionMap: TerminalSessionMap,
  session: TerminalSession,
  sessionKey: string,
  ...
  endShellSession(deps, sessionMap, session, sessionKey);
  broadcastClosed(session, exitCode);
  // Nothing may emit to those panes again. `deleteByKey` (inside
  // endShellSession) already dropped every binding; clearing the registry
 ...
 *
 * Module-level (not a closure inside `buildShellHandlers`) because two
 ...
 * remains: a later connect on that same socket REUSING the evicted
 * connectionId hits the "already in use" guard until the pane disconnects —
 * the real client never does this (a fresh UUID per mount, and a dead pane
 * refuses to re-bind).
 */
function removeViewer(
  ...
  // loop (sprites-shell.ts's `planWatchdogResponse`) so an idle detached shell
  // stops waking the Sprite every ~45s for nobody. This only quiets OUR exec
  // connection — an agent still running inside the shell keeps running,
  // exactly as it already does across the 30-min idle reap below.
 ...
 *   - a HEADLESS create (`ensureShellSession`), which installs a session with
 *     zero viewers and so never reaches that transition at all. Without an arm
 *     here its PTY, slot and billing heartbeat would run for the life of the
 *     process;
 *   - agent input into a viewer-less session (`shell-io.ts`), which RE-arms:
...
export function armIdleReap(
  ...
  session.idleTimer = setTimeout(() => {
    ...
    endShellSession(deps, sessionMap, session, sessionKey);
    loggers.realtime.info('Shell session reaped (idle)', { sessionKey, sandboxId: session.sandboxId });
  }, DETACHED_IDLE_MS);
}
 ...
 *
 * The notification is `shell:error` with a reason, NOT `shell:closed`: closed
 * means "the process exited", and for an eviction that is false — the agent is
 * still running and other panes still show it live. The client treats both the
 * same way mechanically (pane goes dead, editing lock released), so the only
 * difference is that the user reads the truth ("access revoked") instead of a
 * phantom crash ("exited with code -2") for a process that did not exit.
 */
...
async function settleAccruedWindow(
  ...
  //
  // Skipping the gate does mean an insolvent payer's quiesced shell is not
  ...
  // insolvency already carries, not a new one: the settle cadence, not this
  // branch, is what bounds it.
  if (stopClock) {
  ...
  try {
    ...
    loggers.realtime.error('Shell heartbeat re-hold failed', error instanceof Error ? error : new Error(String(error)), {
      sessionKey,
    });
  }
  return true;
  ...
  const interval = setInterval(async () => {
    ...
    try {
      ...
      if (!solvent && sessionMap.getByKey(sessionKey) === session) {
        ...
        teardownShellSession({ billing, persistColdTail }, sessionMap, session, sessionKey, -2);
      }
    } finally {
      settling = false;
    }
...
/**
 * Arms the platform-task-hold heartbeat: one immediate tick — the viewer is
 * attached, so the hold must exist NOW, not a cadence from now — then one tick
 * per controller cadence (the documented 60s refresh against a 5m expiry) for
 * as long as the session is live. Each tick re-reads the session's CURRENT
 * attached/output state, so the same beat that refreshes a busy session's hold
 * deletes an idle one's. A module-level factory for the same reason as
 * `startSettleHeartbeat`: the long-lived callback captures only what it needs,
 * not the whole connect scope.
 */
function startTaskHoldHeartbeat(
  ...
  const tick = () =>
    taskHold.tick({
      ...
      // viewer", not merely "a browser tab is open". A quiesced socket
      // (`isQuiesced`) is deliberately DOWN, so it gives the platform nothing
      // to keep the sandbox resident FOR, and holding the sprite up for it is
      // exactly the idle-cost bug this signal exists to close. Without this,
      // `planHold`'s `needHold = attached || agentRunning` would refresh the
      // hold forever behind any open tab — the watchdog would have gone quiet
      // while the hold went on pinning the Sprite, and the RAM bill would not
      // move.
      //
      ...
      // deliberately never reconnects it — so the clock can freeze under an
      // agent that is still working. That is blind, and the controller then
      // keeps an existing hold rather than deleting it under an agent it can
      // no longer see. And a detached agent that FINISHES while the socket
 ...
 * because `finishCreate()` never runs — every subsequent connect for this shell
...
/** The address a session is created for — the same `{shellId}` `checkAuth` takes. */
...
export interface EnsureShellSessionRequest {
  ...
  userId: string;
  /** Already clamped by the caller (the contract schema clamps on parse). */
  cols: number;
  rows: number;
  /**
   ...
   * an agent nobody will ever see. For a headless start, this is the HTTP
   * request's own connection — the web tier's `fetch` to this endpoint times
   * out sooner than a cold Sprite wake can finish, and without tracking that,
   * a client who gave up could see its retried input executed twice on a
   * create that finished after the fact. Addressed by KEY regardless: the next
   * `send_shell` finds whatever this create did or didn't leave behind.
   */
  abandoned: () => boolean;
  /**
...
export type EnsureShellSessionResult =
...
/**
 * Get a live PTY session for this shell, creating one if there is none.
 *
 ...
 * socket caller turns those into `shell:error`/`ready`; the headless caller
 * turns them into an HTTP answer.
 */
export async function ensureShellSession(
  ...
  // your session": `shell-io.ts` would go on to WRITE into it, and a retry
  ...
  try {
    ...
    // reattach above returned without ever calling this. This runs the SHARED
    // `ensureAgentSessionSandbox` path — one code path with the web tier, so
    // concurrent provisioners CAS against the same store instead of fighting.
    ...
    // Metering: place a flat-estimate hold for this NEW active window BEFORE
    // opening the shell (hibernated/idle time between sessions is free).
    // Settled at session end — see endShellSession. Deliberately NOT in
    // checkAuth: that also runs on every periodic re-auth tick below, which
    ...
    if (billing) {
      ...
      // failure would lock a free-tier user (limit 1) out of shells on this
    ...
    // reap is armed for DETACHED_IDLE_MS, so a pane closed during a cold boot
    // would hold a concurrency slot and bill the session's payer for thirty
    // minutes of Sprite runtime for an agent nobody ever saw. On the free tier
    // (one shell) that is a thirty-minute lockout — a smaller version of the
    // very harm this machinery exists to prevent.
    //
    ...
    try {
      shell = openShell({
        ...
        onExit: (exitCode) => {
          session.releaseSlot();
          loggers.realtime.info('Shell session closed', { exitCode, sandboxId, sessionKey });
          broadcastClosed(session, exitCode);
          // Clears all of the session's timers (re-auth, settle heartbeat, idle).
          endShellSession(sessionEndDeps, sessionMap, session, sessionKey);
        },
        ...
        // could not guarantee on a Sprite with concurrent shells.
        //
    ...
    // attached, and an agent reading this session's bytes over `read_shell`
    ...
    // verdict, and the next `send_shell` resumes it — so keeping it attached
    ...
    // Sprites Tasks API hold: tell the platform work is in progress so the
    // sprite can't cold-pause an agent mid-run — and stop telling it the
    // moment nothing is (see startTaskHoldHeartbeat). Armed only on the cold
    // path: a reattach joins a session whose controller and heartbeat already
    // live for exactly as long as the session does (cleared in
    // endShellSession).
    if (createTaskHold) {
    ...
    // session is torn down IMMEDIATELY — no one-tick grace for an
    // unsupervised revoked process.
    //
      ...
      try {
        ...
        await Promise.all(
          userIds.map(async (uid) => {
            try {
              const result = await checkAuth({ userId: uid, shellId });
              ...
              loggers.realtime.warn('Shell re-auth check failed; keeping viewer this tick', {
                sessionKey,
        ...
        for (const [viewerKey, attached] of attachedViewers) {
          ...
          evictViewer(sessionEndDeps, sessionMap, liveSession, viewerKey, attached, 'Session access revoked');
        }
        ...
        // harm the multi-viewer change removed. If viewers exist now, they
        // were either checked this tick (and the revoked ones evicted) or
        // they joined mid-check and are checked next tick.
        if (liveSession.viewers.size === 0 && revoked.has(liveSession.lastViewerUserId)) {
          teardownShellSession(sessionEndDeps, sessionMap, liveSession, sessionKey, -2);
        }
      } finally {
        reauthing = false;
      }
 ...
 * verbatim — each is the `shell:*` family's spelling of the message the
 * predecessor emitted, so the Phase 6 client matches on exactly these.
 *
 ...
 */
export function connectFailureMessage(failure: Extract<EnsureShellSessionResult, { kind: 'failed' }>): string | undefined {
...
export function buildShellHandlers({
  ...
  /**
   * Every connectionId this SOCKET currently has a live shell session under.
   * `buildShellHandlers` is instantiated once per socket connection (see
   * `apps/realtime/src/index.ts`), so this closure is exactly as socket-scoped
   * as `socket` itself — a real socket disconnect (browser tab closed/reloaded)
   * needs to tear down ALL of them, not just one pane's.
   */
   ...
   * create itself (provisioning or waking a Sprite, asking which sessions it
   * has, exec'ing the agent). Seconds, on a cold boot.
   *
   * A pane that goes away inside that window (the tab closed, the user switched
   * views, StrictMode's double-mount) sends its `shell:disconnect` BEFORE the
   * connect has registered anything to disconnect. Without this set the message
   * lands on nothing and the connect finishes into the void — and EVERY exit
   * from that void leaves a live PTY with no viewer:
   *
   ...
   * the life of the process. On the free tier (one shell) that locks the user out.
   *
   ...
   *     concurrency slot and bill the session's payer for thirty minutes of Sprite
  ...
  const makeViewer = (connectionId: string, userId: string): TerminalViewer => ({
    userId,
    emitOutput: (data) => socket.emit('shell:output', { data, connectionId }),
    emitClosed: (exitCode) => socket.emit('shell:closed', { exitCode, connectionId }),
    emitError: (message) => socket.emit('shell:error', { message, connectionId }),
  });
  ...
  async function establishConnection(value: ShellConnectPayload, connectionId: string) {
    ...
    // The access check is DB-only, reserves no concurrency slot, and the
    // session key derives from the shellId without the Sprite — so the live
    ...
    const accessResult = await checkAuth({ userId, shellId });
    ...
    if (plan.kind === 'deny') {
      socket.emit('shell:error', { message: `Shell access denied: ${plan.reason}`, connectionId });
      return;
    }
    ...
    // (`ensureShellSession`). What this connect contributes is the two
    ...
    const outcome = await ensureShellSession(
      { sessionMap, openShell, checkAuth, persistSpriteExecId, billing, createTaskHold, persistColdTail },
      {
        ...
        onStarted: (resumed) => {
          ...
          // starting prompt into the shell has to know the difference — a
          ...
          socket.emit('shell:ready', { connectionId, resumed });
        },
      },
    );
    ...
    if (outcome.kind === 'failed') {
      ...
      if (message !== undefined) socket.emit('shell:error', { message, connectionId });
      return;
    }
  }
  ...
  return {
    async onConnect(payload: unknown) {
      const validation = parseShellConnectPayload(payload);
      if (!validation.ok) {
      ...
      if (activeConnectionIds.has(connectionId) || connectingConnectionIds.has(connectionId)) {
        socket.emit('shell:error', { message: 'Shell connectionId already in use', connectionId });
        return;
      }
    ...
    onResize(payload: unknown) {
      ...
      if (typeof p?.cols === 'number' && typeof p?.rows === 'number' && Number.isFinite(p.cols) && Number.isFinite(p.rows)) {
        const { cols, rows } = clampShellDimensions({ cols: p.cols, rows: p.rows });
    ...
    onDisconnect(payload?: unknown) {
      ...
      if (explicitConnectionId !== undefined) {
        // One pane explicitly closed — leave this socket's OTHER connections
        // (other live panes) untouched. Only a connectionId THIS socket itself
        // registered via a successful, authorized `onConnect` is honored — the
        // session map is one shared, server-wide instance (every socket's
        // sessions live in it), so a connectionId a client merely CLAIMS
        // (rather than one this socket actually established) must never be
        // trusted to reach another socket's PTY.
