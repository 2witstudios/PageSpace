 ...
 *
 * This is `apps/web/src/stores/__tests__/usePendingStreamsStore.test.ts` with EXACTLY ONE
 ...
 * in `createPendingStreamsFacade`. Every `it(...)` title, every arrangement and
...
import { createPendingStreamsFacade, type PendingStreamsFacade } from '../facade/pendingStreamsFacade';

let usePendingStreamsStore: PendingStreamsFacade;
...
describe('usePendingStreamsStore', () => {
  beforeEach(() => {
    usePendingStreamsStore = createPendingStreamsFacade(createChatDatabase().db);
  });
