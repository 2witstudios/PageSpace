...
export function useSideQuestion(conversationId: string) {
  ...
  const ask = useCallback(async (question: string) => {
    ...
    try {
      const response = await fetch('/api/ai/btw', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Btw-Request-Id': request.requestId }, body: JSON.stringify(request.body), signal: request.controller.signal });
