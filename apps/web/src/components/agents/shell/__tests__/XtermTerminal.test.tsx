...
// xterm is a DOM/canvas library the shell imports dynamically; stub it so what's
...
let disposals = 0;
/** The shell's `terminal.onData` callback, captured so a test can simulate the
...
function fakeSocket() {
  // A SET of handlers per event, and a working `off` — every shell in the
  // session registers on this one socket, so a Map of one handler per event
  // could never hold two shells at once, and a no-op `off` could never catch
  // a teardown that failed to unregister. Both are the machinery the prompt
  // latches rest on.
  ...
  return {
    ...
    /** The connectionIds the mounted shells announced — every event is tagged with one. */
    connectionIds: () =>
      emitted
        .filter((entry) => entry.event === 'shell:connect')
        .map((entry) => entry.payload.connectionId as string),
    connectionId: () => emitted.find((entry) => entry.event === 'shell:connect')?.payload.connectionId,
...
const inputs = (emitted: Array<{ event: string; payload: Record<string, unknown> }>) =>
  emitted.filter((entry) => entry.event === 'shell:input').map((entry) => entry.payload.data);
...
describe('XtermTerminal — starting-prompt delivery', () => {
  ...
  test('an agent that boots silently still gets its prompt, via the backstop', async () => {
    const fake = fakeSocket();
    render(<XtermTerminal socket={fake.socket} shellId="shell-1" initialInput="fix the build" />);
    await waitFor(() => expect(fake.hasHandlers()).toBe(true));

    fake.server('shell:ready', { connectionId: fake.connectionId() });
  ...
  test('an agent the bridge RESUMED is never typed at, even though the connect looks cold', async () => {
    ...
    fake.server('shell:ready', { resumed: true, connectionId });
    fake.server('shell:output', { data: 'y/n? ', connectionId });
    await vi.advanceTimersByTimeAsync(5000);
  ...
  test('a re-mount onto a still-silent booting agent DOES deliver — StrictMode must not eat the prompt', async () => {
    ...
    // A reattach to a PTY that has emitted NOTHING: the boot this shell is
    // waiting for, reached through a re-mount. React StrictMode does exactly
    // this in dev, so treating every reattach as unsafe would mean the prompt
    // never worked while developing the feature.
    fake.server('shell:ready', { scrollback: '', resumed: false, connectionId });
    fake.server('shell:output', { data: 'shell> ', connectionId });
  ...
  test('a REATTACHED agent that has already printed is never typed at', async () => {
    ...
    // shell connected — the prompt belongs to a boot that already happened.
    fake.server('shell:ready', { scrollback: '$ ls\nREADME.md\n', connectionId });
    fake.server('shell:output', { data: 'y/n? ', connectionId });
    await vi.advanceTimersByTimeAsync(5000);

    assert({
      given: 'a shell reattaching to an agent that has been running (a reload, a tab switch, days later)',
      should:
  ...
  test('a prompt is delivered at most once, however many ready/output events arrive', async () => {
    ...
    fake.server('shell:ready', { connectionId });
    fake.server('shell:output', { data: 'a', connectionId });
    fake.server('shell:output', { data: 'b', connectionId });
    fake.server('shell:ready', { connectionId });
    await vi.advanceTimersByTimeAsync(5000);
  ...
  test('an event for ANOTHER shell on the shared socket is ignored', async () => {
    const fake = fakeSocket();
    render(<XtermTerminal socket={fake.socket} shellId="shell-1" initialInput="fix the build" />);
    await waitFor(() => expect(fake.hasHandlers()).toBe(true));

    // Every shell in the session shares one socket, so it sees its siblings' events.
    fake.server('shell:ready', { connectionId: 'another-shell' });
    fake.server('shell:output', { data: 'not mine', connectionId: 'another-shell' });
    await vi.advanceTimersByTimeAsync(5000);

    assert({
      given: "a sibling shell's ready and output arriving on the shared socket",
  ...
  test('a shell with no starting prompt types nothing', async () => {
    const fake = fakeSocket();
    render(<XtermTerminal socket={fake.socket} shellId="shell-1" />);
    await waitFor(() => expect(fake.hasHandlers()).toBe(true));

    fake.server('shell:ready', { connectionId: fake.connectionId() });
    fake.server('shell:output', { data: 'shell> ', connectionId: fake.connectionId() });
    await vi.advanceTimersByTimeAsync(5000);

    assert({
      given: 'a shell opened with no starting prompt (the field is optional)',
  ...
  test('a shell unmounted mid-boot types nothing more, and does NOT spend the prompt', async () => {
    const fake = fakeSocket();
    const onSent = vi.fn();
    ...
    assert({
      given: 'a shell closed (or a tab switched away from) while its agent was still booting',
      should:
      ...
      actual: { typed: inputs(fake.emitted), promptSpent: onSent.mock.calls.length, listeners: fake.listenerCount('shell:output') },
  ...
  test('a FRESH agent whose output beat its ready is prompted as soon as ready lands', async () => {
    ...
    fake.server('shell:output', { data: 'shell> ', connectionId });
    fake.server('shell:ready', { resumed: false, connectionId });
  ...
  test('two shells on ONE socket: only the prompted shell is typed at, and closing one leaves the other alive', async () => {
    // Every shell in the session shares a single socket, so each sees the
    // others' events. This is the machinery `isMine` and the per-mount latches
    // rest on — and until now the fake socket could not even hold two shells at
    // once.
    const fake = fakeSocket();
    const shellA = render(
      <XtermTerminal socket={fake.socket} shellId="a" initialInput="fix the build" />
    );
    render(<XtermTerminal socket={fake.socket} shellId="b" />);
    ...
    fake.server('shell:ready', { connectionId: idB });
    fake.server('shell:output', { data: 'other> ', connectionId: idB });
    const afterBOnly = inputs(fake.emitted);

    // Now A's own agent comes up.
    fake.server('shell:ready', { connectionId: idA });
    fake.server('shell:output', { data: 'shell> ', connectionId: idA });

    shellA.unmount();
    const listenersAfterAClosed = fake.listenerCount('shell:output');

    assert({
      given: "two shells multiplexed on one socket, only one of them holding a starting prompt",
      should:
  ...
  test('output that flows without a ready is never typed at, and the prompt is not spent', async () => {
    ...
    fake.server('shell:output', { data: 'something', connectionId: fake.connectionId() });
    await vi.advanceTimersByTimeAsync(5000);

    assert({
      given: 'a shell receiving output but never a ready',
...
describe('XtermTerminal — PTY binding across socket reconnects', () => {
  ...
  test('a transport reconnect re-fires shell:connect exactly once, on the shell’s own connectionId', async () => {
    const fake = fakeSocket();
    render(<XtermTerminal socket={fake.socket} shellId="shell-1" />);
    ...
    // object, so the [socket, shellId]-keyed mount effect never re-runs —
    // the `connect` event is the only signal a re-bind can hang off.
    fake.reconnect();

    assert({
      given: 'a live shell whose socket dropped its transport and reconnected',
      should:
        're-emit shell:connect exactly once, reusing the shell’s connectionId, without recreating the terminal — the server side of the binding died with the transport, and without this the shell looks alive until the idle reap kills its agent',
      actual: {
  ...
  test('the initial connection binds exactly once, wherever mount falls relative to it', async () => {
    ...
    render(<XtermTerminal socket={early.socket} shellId="shell-1" />);
    ...
    render(<XtermTerminal socket={late.socket} shellId="shell-2" />);
    ...
    assert({
      given: 'one shell mounted after its socket connected, another mounted before',
      should: 'emit exactly one shell:connect each — never zero, never two',
  ...
  test('a reconnect reattach repaints from the replayed scrollback instead of appending it under what is shown', async () => {
    ...
    await waitFor(() => expect(fake.connectionIds().length).toBe(1));
    const connectionId = fake.connectionId();
    fake.server('shell:ready', { scrollback: '$ bun run build\n', connectionId });
    fake.server('shell:output', { data: 'compiling…\n', connectionId });
    ...
    fake.server('shell:ready', {
      scrollback: '$ bun run build\ncompiling…\ndone\n',
      resumed: true,
      connectionId,
    });
    // A later duplicate ready on the SAME connection must not wipe live output.
    fake.server('shell:output', { data: '$ ', connectionId });
    fake.server('shell:ready', { connectionId });

    assert({
      given: 'a shell with content on screen whose re-bind is answered with the full scrollback replay',
      should:
  ...
  test('a re-bind ready WITHOUT a replay keeps the buffer — resetting would blank a shell full of history', async () => {
    const fake = fakeSocket();
    render(<XtermTerminal socket={fake.socket} shellId="shell-1" />);
    await waitFor(() => expect(fake.connectionIds().length).toBe(1));
    const connectionId = fake.connectionId();
    fake.server('shell:ready', { scrollback: '$ ls\n', connectionId });
    fake.server('shell:output', { data: 'README.md\n', connectionId });
  ...
  test('a shell whose process EXITED never auto-rebinds — a reconnect must not resurrect a finished agent', async () => {
    const fake = fakeSocket();
    render(<XtermTerminal socket={fake.socket} shellId="shell-1" />);
    await waitFor(() => expect(fake.connectionIds().length).toBe(1));
    const connectionId = fake.connectionId();
    fake.server('shell:ready', { connectionId });
    fake.server('shell:closed', { exitCode: 0, connectionId });

    const printedBeforeDrop = written.length;
    fake.reconnect();

    assert({
      given: 'a shell showing "process exited" whose socket then drops and reconnects',
      should:
        'emit no further shell:connect and print no reconnecting promise — the server has no session left under this key, so a re-bind would take its cold CREATE path and silently launch (and bill) a brand-new agent nobody asked for',
  ...
  test('a shell the server REFUSED never auto-rebinds — reconnect cycles must not retry a denied bind', async () => {
    const fake = fakeSocket();
    render(<XtermTerminal socket={fake.socket} shellId="shell-1" />);
    await waitFor(() => expect(fake.connectionIds().length).toBe(1));
    const connectionId = fake.connectionId();
    fake.server('shell:error', { message: 'Shell access denied', connectionId });

    const printedBeforeDrop = written.length;
    fake.reconnect();

    assert({
      given: 'a shell whose bind the server denied (access revoked, insufficient credits), riding out a transport cycle',
      should:
  ...
  test('a shell that goes dead (exited, or refused) releases its editing-store registration immediately', async () => {
    const fake = fakeSocket();
    render(<XtermTerminal socket={fake.socket} shellId="exited-shell" />);
    await waitFor(() => expect(fake.connectionIds().length).toBe(1));
    const connectionId = fake.connectionId();
    fake.server('shell:ready', { connectionId });
    expect(editingStore.startEditing).toHaveBeenCalledWith('exited-shell', 'shell', { componentName: 'shell' });

    fake.server('shell:closed', { exitCode: 0, connectionId });

    assert({
      given: 'a shell whose agent process exits while the shell stays mounted (dead shells never unmount on their own)',
      should:
        'release the editing-store registration right away — a finished shell must not go on blocking auth refresh/SWR updates for its shellId until some later, unrelated unmount',
      actual: editingStore.endEditing.mock.calls,
      expected: [['exited-shell']],
    });
  });
