 ...
 */
export interface MachinePageNode {
  id: string;
  type: string;
  children?: readonly MachinePageNode[] | null;
}
 ...
 */
export function collectMachinePageIds(
  tree: readonly MachinePageNode[] | null | undefined,
): string[] {
  const ids: string[] = [];
  const walk = (nodes: readonly MachinePageNode[] | null | undefined) => {
    if (!nodes) return;
...
export interface PlanMountedMachinesInput {
 ...
 */
export function planMountedMachines({
  current,
  visited,
  max,
  valid,
}: PlanMountedMachinesInput): string[] {
