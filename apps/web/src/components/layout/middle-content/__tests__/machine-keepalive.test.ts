...
import {
  ...
  type MachinePageNode,
} from '../machine-keepalive';

describe('planMountedMachines', () => {
  it('given no active terminal and no history, should mount nothing', () => {
    assert({
      ...
      actual: planMountedMachines({ current: null, visited: [], max: 3 }),
      expected: [],
    });
  });
  ...
  it('given a first-visited terminal, should mount it', () => {
    assert({
      ...
      actual: planMountedMachines({ current: 'a', visited: [], max: 3 }),
      expected: ['a'],
    });
  });
  ...
  it('given the active terminal already in history, should pin it to the front without duplicating', () => {
    assert({
      ...
      actual: planMountedMachines({ current: 'b', visited: ['a', 'b', 'c'], max: 3 }),
      expected: ['b', 'a', 'c'],
    });
  });
  ...
  it('given a new active terminal beyond the bound, should evict the least-recently-used', () => {
    assert({
      ...
      actual: planMountedMachines({ current: 'd', visited: ['c', 'b', 'a'], max: 3 }),
      expected: ['d', 'c', 'b'],
    });
  });
  ...
  it('given navigation to a non-terminal page, should keep the mounted set intact (no active pin)', () => {
    assert({
      ...
      actual: planMountedMachines({ current: null, visited: ['c', 'b', 'a'], max: 3 }),
      expected: ['c', 'b', 'a'],
    });
  });
  ...
  it('given a bound of zero with an active terminal, should still keep the active one pinned', () => {
    assert({
      ...
      actual: planMountedMachines({ current: 'a', visited: ['b'], max: 0 }),
      expected: ['a'],
    });
  });
  ...
  it('given repeated ids in history, should de-duplicate preserving MRU order', () => {
    assert({
      ...
      actual: planMountedMachines({ current: 'a', visited: ['a', 'b', 'a', 'c'], max: 5 }),
      expected: ['a', 'b', 'c'],
    });
  });
});

describe('collectMachinePageIds', () => {
  const tree: MachinePageNode[] = [
    {
      id: 'root',
      type: 'FOLDER',
      children: [
  ...
  it('given a nested tree, should collect every terminal id at any depth', () => {
    assert({
      ...
      actual: collectMachinePageIds(tree).sort(),
      expected: ['t1', 't2', 't3'],
    });
  });
  ...
  it('given an empty or nullish tree, should return an empty list', () => {
    assert({
      ...
      actual: collectMachinePageIds(null),
      expected: [],
    });
  });
});
