...
describe('GlobalAssistantView load-on-select effects', () => {
  ...
  describe('app-state resume recovery (useAppStateRecovery wiring)', () => {
    ...
    // App-resume itself (the useResumeBootstrap wiring: rejoin → reload → conditional stop,
    // replacing tryRecover/decideRecovery/resolveResumeAction/evictStalePartial and the
    // regenerateTurnOnce mutex — epic leaf 6.2) is a shared hook now. Its decision is pinned
    // by lib/ai/streams/__tests__/planResumeBootstrap.test.ts; render-level verification that
    // GlobalAssistantView wires it correctly is a main-checkout/CI item (render tests don't run
    // in .pu worktrees).
  });
});
