...
import { updateCssVariables, PaginationConfig } from "./utils";
import { PageSize } from "./constants";

const PAGE_COUNT_META_KEY = "PAGE_COUNT_META_KEY";

export interface PaginationPlusOptions extends PaginationConfig {
  pageGap: number;
...
function getExistingPageCount(view: EditorView): number {
...
function calculatePageCount(view: EditorView, pageOptions: PaginationPlusOptions): number {
  const editorDom = view.dom as HTMLElement;
  updateCssVariables(editorDom, pageOptions);

  const _pageHeaderHeight =
    pageOptions.pageHeaderHeight + pageOptions.contentMarginTop + pageOptions.marginTop;
  const _pageFooterHeight =
    pageOptions.pageFooterHeight + pageOptions.contentMarginBottom + pageOptions.marginBottom;
  ...
  if (paginationElement) {
    ...
    if (lastElementOfEditor && lastPageBreak) {
      const lastPageGap =
        lastElementOfEditor.getBoundingClientRect().bottom - lastPageBreak.getBoundingClientRect().bottom;
...
function createDecoration(state: EditorState, pageOptions: PaginationPlusOptions, isInitial = false): Decoration[] {
  const pageWidget = Decoration.widget(
    ...
    (view: EditorView) => {
      ...
      const pageBreakDefinition = ({ firstPage = false }: { firstPage?: boolean }) => {
