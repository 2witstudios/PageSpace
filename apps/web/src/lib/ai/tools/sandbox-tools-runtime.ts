 ...
 * quota/concurrency surface, the audit writer, and the chat-context → actor
 * resolver. Kept in its own module so the factory (and its tests) never import
 * the backing-provider SDK (`@fly/sprites` is ESM/node24-only).
 *
