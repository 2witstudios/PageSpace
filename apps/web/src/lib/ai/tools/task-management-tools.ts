...
export const taskManagementTools = {
  ...
  /**
   * Add a note or progress update to a task
   */
  add_task_note: tool({
    description: 'Add a note or progress update to a specific task. Useful for documenting work done, issues encountered, or decisions made.',
    inputSchema: z.object({
      taskId: z.string().describe('The ID of the task to add a note to'),
      note: z.string().describe('The note or progress update'),
      updateStatus: z.boolean().default(false).describe('Also update task status to in_progress'),
    }),
    execute: async ({ taskId, note, updateStatus }, { experimental_context: context }) => {
      const userId = (context as ToolExecutionContext)?.userId;
      ...
      try {
        ...
        if (updateStatus && task.status === 'pending') {
        ...
        return {
          ...
          message: `Added note to task "${updatedTask.title}"`,
          summary: `Documented progress on "${updatedTask.title}"`,
          statusUpdated: updateStatus && task.status === 'pending',
          nextSteps: [
            'Continue working on the task',
            'Update task status when appropriate',
            'Add more notes as you make progress',
          ]
        };
      } catch (error) {
        console.error('Error adding task note:', error);
        throw new Error(`Failed to add task note: ${error instanceof Error ? error.message : String(error)}`);
      }
    },
  }),
