...
For html-mode DOCUMENTs, the stored HTML is first normalized by an **additive** line-breaker: a newline is added after each opening block tag, before each closing block tag, between adjacent block tags (\`p\`, \`h1\`–\`h6\`, \`ul\`/\`ol\`/\`li\`, \`table\`/\`tr\`/\`td\`/\`th\`/\`thead\`/\`tbody\`, \`blockquote\`, \`pre\`, \`div\`, and similar), and after each \`<br>\`/\`<hr>\` — a \`<br>\` ends a line, so a document laid out with \`<br>\` separators numbers one line per break. Nothing is ever removed or changed: text, spacing and blank lines all survive. So a paragraph reads as three lines:
