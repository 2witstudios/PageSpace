...
- Filters and sorts run in the database against each cell's **computed, unformatted** value, so a formula column matches on its result. \`cells\` gives you the DISPLAY text, so a formatted cell reads back as \`$1,200.00\` while its filter value is \`1200\` — and the row carries that machine value in \`unformatted\` (\`unformatted: { "C": 1200 }\`) for exactly the cells where the two differ. Read the number from there rather than parsing the display string; a column letter absent from \`unformatted\` means \`cells\` already holds the value. Filter, compute and write back on that number, never on the way it is displayed.
- \`includeFormatting: true\` adds a \`formatting\` block: the tab's \`regions\` (its declared structure — which table is where, which rows are headers or totals, what each column means — and re-writable exactly as you read them), \`layout\` (freezes, widths, heights), \`columnFormats\`, the explicit \`cellFormats\` within the rows returned, and a one-line \`summary\` of each conditional rule. Ask for it before you restyle a sheet, so you build on the structure that is there. \`cellFormats\` is capped and reports \`formattingTruncated\` when it drops cells — a missing cell there is not proof it has no format. On a sheet that has not been migrated to row storage you get \`formatting: null\` and \`formattingUnavailable\`: that means the styling is unreadable, **not** that the sheet is unstyled.
