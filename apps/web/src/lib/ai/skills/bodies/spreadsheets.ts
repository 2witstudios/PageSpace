...
- \`includeFormatting: true\` adds a \`formatting\` block: the tab's \`regions\` (its declared structure — which table is where, which rows are headers or totals, what each column means — and re-writable exactly as you read them), \`layout\` (freezes, widths, heights), \`columnFormats\`, the explicit \`cellFormats\` within the rows returned, and a one-line \`summary\` of each conditional rule. Ask for it before you restyle a sheet, so you build on the structure that is there. \`cellFormats\` is capped and reports \`formattingTruncated\` when it drops cells — a missing cell there is not proof it has no format. It works on unmigrated sheets too — their styling is read back out of the stored document, so you never have to guess whether a legacy sheet is already designed.
