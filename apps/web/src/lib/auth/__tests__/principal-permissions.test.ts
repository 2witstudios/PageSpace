...
vi.mock('@pagespace/db/db', () => ({
  db: { query: { pages: { findFirst: (...args: unknown[]) => mockPagesFindFirst(...args) } } },
}));
...
describe('manage-keys-only credential — deny-first short-circuit', () => {
  ...
  const brokenManageKeysAuth = manageKeysScopedAuthResult({
    scopes: { account: true, offlineAccess: false, drives: new Map(), manageKeys: true, allDrives: false, profile: false, updateKeyId: null, activateKeyId: null, newKeyName: null },
