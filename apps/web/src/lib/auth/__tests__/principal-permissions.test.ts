...
const scopedOAuthAuth: AuthResult = {
  ...base, userId: USER_ID, tokenType: 'oauth', tokenId: TOKEN_ID,
  scopes: { account: false, offlineAccess: false, drives: new Map([[DRIVE_ID, { kind: 'drive' as const, driveId: DRIVE_ID, role: { kind: 'inherit' as const } }]]), manageKeys: false, updateKeyId: null, activateKeyId: null },
...
describe('manage-keys-only credential — deny-first short-circuit', () => {
  ...
  const brokenManageKeysAuth = manageKeysScopedAuthResult({
    scopes: { account: true, offlineAccess: false, drives: new Map(), manageKeys: true, updateKeyId: null, activateKeyId: null },
