...
export class IOSStorage implements PlatformStorage {
  ...
  async getSessionToken(): Promise<string | null> {
    const { getSessionToken } = await import('@/lib/native-google-auth');
    return getSessionToken();
  }

  async getStoredSession(): Promise<StoredSession | null> {
    const { getStoredSession } = await import('@/lib/native-google-auth');
  ...
  async clearSession(): Promise<void> {
    const { clearStoredSession } = await import('@/lib/native-google-auth');
