...
import { envStorageMeasureSeam } from '@pagespace/lib/services/drive-envs/env-storage-measure';
...
function buildEnvProvisionDeps({
  ...
  return {
     ...
     *
     * The seam itself lives in `@pagespace/lib`
     * (`services/drive-envs/env-storage-measure.ts`) rather than as a closure
     * here: WHICH row a measurement lands on is the holder kind's answer, and it
     * is the same answer in every process that provisions an env. Wired in one
