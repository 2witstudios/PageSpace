 ...
 * `{ action: 'stop' | 'resume' }`, `{ action: 'approve', port, spriteInstanceId }`,
 * or `{ action: 'select', port, spriteInstanceId }`. One parser for the
 * session and env routes so "well-formed" cannot drift between them.
 *
 * `approve` and `select` carry the same fields and differ in what they
 * assert: approve agrees to share the port DETECTION found and the row
 * already targets; select names a port by hand, which the row may not
 * target and may never have seen. The store treats them differently for
 * exactly that reason (`approvePort` vs `selectPort`).
 *
...
export function readDevPreviewUserAction(body: unknown): DevPreviewUserAction | null {
  ...
  if (action !== 'approve' && action !== 'select') return null;
  ...
  return { kind: action, port, spriteInstanceId };
}
