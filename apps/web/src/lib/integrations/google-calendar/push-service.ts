...
export const buildGoogleRRule = (
  ...
  if (rule.until) {
    // Convert ISO string to Google's UNTIL format (YYYYMMDDTHHMMSSZ)
 ...
 */
export const transformPageSpaceEventToGoogle = (event: CalendarEvent): GoogleEventWriteBody => {
 ...
 * Returns the connection details needed for push operations.
 */
export const getActiveConnection = async (
  ...
  return {
    connected: true,
    // Push is enabled when markAsReadOnly is false (bidirectional sync)
    pushEnabled: !connection.markAsReadOnly,
    // Push to the first selected calendar, or 'primary' as fallback
    targetCalendarId: connection.selectedCalendars?.[0] || 'primary',
  };
};
...
export const pushEventToGoogle = async (
  ...
  try {
    ...
    if (!connection?.pushEnabled) {
      return { success: true }; // Silently skip if push not enabled
    }
...
export const pushEventDeleteToGoogle = async (
  ...
  try {
    ...
    // Fetch before soft-delete changes take effect (caller should call this before trashing)
    const event = await db.query.calendarEvents.findFirst({
      where: eq(calendarEvents.id, eventId),
    });
