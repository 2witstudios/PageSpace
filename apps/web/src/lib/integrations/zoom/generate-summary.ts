...
export async function generateTranscriptSummary(
  ...
  try {
    return await withZoomAiCredit(userId, 'zoom_summary', () => summarize(userId, transcriptPlainText), '');
  } catch (err) {
...
async function summarize(userId: string, transcriptPlainText: string): Promise<string> {
