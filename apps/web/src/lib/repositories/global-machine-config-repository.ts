/**
 * Global assistant Machine Access configuration.
 *
 * The global assistant's `machineAccess`/`machines[]` parallel to
 ...
 * lazily-provisioned "own machine" Machine page lives (see
...
export interface GlobalMachineConfig {
...
function toConfig(row: { machineAccess: boolean; machines: unknown }): GlobalMachineConfig {
  return {
...
export const globalMachineConfigRepository = {
  async getConfig(userId: string): Promise<GlobalMachineConfig> {
    const row = await getOrCreateConfig(db, userId);
    return toConfig(row);
  },

  /** Machine pages in the user's Home drive they can see, for the "use existing machine" picker. */
  async getAvailableMachines(userId: string): Promise<Array<{ id: string; title: string }>> {
  ...
  /**
   * Get-or-create the personal Machine page backing this user's "own"
   ...
   * the same user, so two machine-tool calls racing to provision the "own"
