...
const LEGACY_AI_WORKFLOW = { steps: null, prompt: 'Summarize', agentPageId: 'agent_1' };
...
describe('acquireWorkflowCreditHold', () => {
  ...
  it('gates the billed user at their tier before any model runs', async () => {
    ...
    const hold = await acquireWorkflowCreditHold('user_1', LEGACY_AI_WORKFLOW, 'scheduled');
  ...
  it('falls back to the free tier when the user row is missing', async () => {
    ...
    await acquireWorkflowCreditHold('user_1', LEGACY_AI_WORKFLOW, 'scheduled');
  ...
  it('sizes the reservation by the number of ai steps in the chain', async () => {
    ...
    await acquireWorkflowCreditHold('user_1', { steps, prompt: '', agentPageId: null }, 'scheduled');
  ...
  it('manual runs keep the daily cap and pass the interactive in-flight cap', async () => {
    ...
    await acquireWorkflowCreditHold('user_1', LEGACY_AI_WORKFLOW, 'interactive');
  ...
  it('a deterministic-only chain runs no model, so it takes no gate and no hold', async () => {
    const steps = [{ kind: 'tool', toolName: 'send_channel_message', args: {} }];

    const hold = await acquireWorkflowCreditHold('user_1', { steps, prompt: '', agentPageId: null }, 'scheduled');
