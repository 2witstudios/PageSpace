 ...
 * `conversations.closedInWorkspaceAt` — a fact kept separate from `isActive`
...
export interface CloseConversationInSessionDeps {
   ...
   * `workspaceId` null or mismatched = an address that names nothing THIS
  ...
  findConversation: (conversationId: string) => Promise<{
    ...
    /**
     * History soft-delete (separate from `closedInWorkspaceAt` — see the
  ...
  /**
   * The guarded UPDATE: stamp `closedInWorkspaceAt` WHERE it is still NULL.
