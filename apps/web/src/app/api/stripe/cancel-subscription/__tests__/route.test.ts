...
describe('POST /api/stripe/cancel-subscription', () => {
  ...
  it('should log audit event on successful cancellation', async () => {
    ...
    expect(auditRequest).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ eventType: 'data.delete', userId: mockUserId, resourceType: 'subscription', resourceId: 'sub_123', details: expect.objectContaining({ action: 'cancel' }) })
    );
  });
