...
import { authenticateRequestWithOptions, isAuthError, checkMCPCreateScope, isDriveScopedPrincipal, getAllowedDriveIds, getPrincipalDriveMembership, isPrincipalDriveMember, getPrincipalDriveAccessLevel } from '@/lib/auth';
...
describe('GET /api/drives — scoped MCP canCreatePages', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(getAllowedDriveIds).mockReturnValue(['drive_scoped']);
  ...
  it('given a scoped token MEMBER with a view-only custom role, should fail closed', async () => {
    vi.mocked(getPrincipalDriveMembership).mockResolvedValue({ role: 'MEMBER', customRoleId: 'role_view' });
  ...
  it('given an inherited scope whose owner cannot edit the drive root, should resolve through the credential drive resolver and fail closed', async () => {
