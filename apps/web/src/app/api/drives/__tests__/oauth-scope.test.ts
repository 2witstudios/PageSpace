  ...
  it('returns no drives for an empty drive-scoped OAuth credential instead of falling back to the full list', async () => {
    vi.mocked(authenticateRequestWithOptions).mockResolvedValue(
      driveScopedOAuthAuthResult({
        scopes: { account: false, offlineAccess: false, manageKeys: false, allDrives: false, profile: false, drives: new Map(), updateKeyId: null, activateKeyId: null, newKeyName: null },
        driveScopes: [],
        allowedDriveIds: [],
      }),
    );
