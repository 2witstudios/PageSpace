...
export async function PATCH(
  ...
  try {
    ...
    if (role && role !== oldRole) {
      ...
      audit({ eventType: 'authz.role.assigned', userId: currentUserId, resourceType: 'drive', resourceId: driveId, details: { targetUserId: userId, previousRole: oldRole, newRole: role } });
    }
    ...
    if (permissions.length > 0) {
      audit({ eventType: 'authz.permission.granted', userId: currentUserId, resourceType: 'drive', resourceId: driveId, details: { targetUserId: userId, permissionsUpdated: permissions.length } });
    }
...
export async function DELETE(
  ...
  try {
    ...
    audit({ eventType: 'authz.permission.revoked', userId: currentUserId, resourceType: 'drive', resourceId: driveId, details: { targetUserId } });
