...
export async function GET(request: Request, context: { params: Promise<{ driveId: string; envId: string }> }) {
  try {
    ...
    const canManage = decideDevPreviewManage({ holder: { kind: 'env', id: envId }, userId: auth.userId, sessionOwnerId: null, isDriveOwnerOrAdmin: await isPrincipalDriveOwnerOrAdmin(auth, driveId) });
