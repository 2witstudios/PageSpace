...
import { readWorkspaceGridsBulk, workspaceListEntryFromGrid } from '@/lib/agent-sessions/workspace-layout-runtime';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [shellsBySession, conversationsBySession, gridBySession] = await Promise.all([
      ...
      readWorkspaceGridsBulk(sessionIds),
    ]);
    const withChildren = sessions.map((session) => ({
      ...
      workspace: workspaceListEntryFromGrid(
        session.sessionId,
        gridBySession.get(session.sessionId) ?? null,
      ),
    }));
