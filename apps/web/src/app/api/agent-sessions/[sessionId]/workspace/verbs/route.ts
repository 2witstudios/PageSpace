...
import { workspaceLayoutVerbSchema } from '@pagespace/lib/agent-sessions/workspace-layout-verbs';
import { checkSessionAccess } from '@/lib/agent-sessions/agent-sessions-runtime';
import { applyWorkspaceLayoutVerb } from '@/lib/agent-sessions/workspace-layout-runtime';
import { sessionNotFoundOrDenied } from '@/lib/agent-sessions/session-unavailable-response';

const AUTH_OPTIONS_WRITE = { allow: ['session'] as const, requireCSRF: true };

const ROUTE = 'agent-sessions/[sessionId]/workspace/verbs';
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    loggers.api.error('Workspace layout verb failed', error instanceof Error ? error : undefined, { sessionId });
