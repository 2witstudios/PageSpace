 ...
 *   — it is no longer a stored blob (the `agent_sessions.workspaceState`
...
} from '@/lib/agent-sessions/workspace-layout-runtime';
...
export async function GET(request: Request, context: RouteContext) {
  ...
  const snapshot = await readWorkspaceLayoutSnapshot(sessionId);
  return NextResponse.json({
    workspace: workspaceListEntryFromGrid(sessionId, snapshot.grid),
    rev: snapshot.rev,
    grid: snapshot.grid,
  });
}
