...
const SHELL = { shellId: SHELL_ID, sessionId: SESSION_ID, ownerId: 'user-1', name: 'shell-1', agentType: 'shell', command: null, createdAt: 'x' };

const del = (sessionId = SESSION_ID, shellId = SHELL_ID) =>
  DELETE(new Request(`http://localhost/api/agent-sessions/${sessionId}/shells/${shellId}`, { method: 'DELETE' }), {
    params: Promise.resolve({ sessionId, shellId }),
  });

beforeEach(() => {
  vi.clearAllMocks();
  mockAuthenticateRequest.mockResolvedValue(AUTH_USER);
  mockResolveShellById.mockResolvedValue({ ok: true, shell: SHELL, cwd: '/workspace', streamSessionId: null });
  mockCheckSessionEndAccess.mockResolvedValue({ allowed: true });
  mockKillShellById.mockResolvedValue({ ok: true, killed: true });
});

describe('DELETE /api/agent-sessions/[sessionId]/shells/[shellId]', () => {
  ...
  it('given a shell under a DIFFERENT session than the URL claims, should 404 without leaking the pairing', async () => {
    mockResolveShellById.mockResolvedValue({
      ok: true,
      shell: { ...SHELL, sessionId: 'someone-elses-session' },
      cwd: '/workspace',
