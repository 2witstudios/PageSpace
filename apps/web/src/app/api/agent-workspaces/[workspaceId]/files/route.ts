 ...
 * branchName?)` tuple to `[workspaceId]` + an optional caller-supplied
...
async function authorize(
  ...
  auditSessionAccessDenial(request, userId, workspaceId, access.reason, ROUTE);
...
function auditWrite(request: Request, userId: string, workspaceId: string, details: Record<string, unknown>): void {
...
export async function GET(request: Request, context: RouteContext) {
  ...
  const deniedResponse = await authorize(request, auth.userId, workspaceId);
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (kind.value === 'directory') {
    ...
    auditWrite(request, auth.userId, workspaceId, { op: 'create_directory', path: rawPath.value });
  ...
  if (!result.ok) return mutateFailureResponse(result, 'The parent folder could not be found');
  auditWrite(request, auth.userId, workspaceId, { op: 'write_file', path: rawPath.value });
  return NextResponse.json({ ok: true });
}
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  auditWrite(request, auth.userId, workspaceId, { op: op.value, fromPath: rawFromPath.value, toPath: rawToPath.value });
  return NextResponse.json({ ok: true });
}
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  auditWrite(request, auth.userId, workspaceId, { op: 'delete', path: rawPath.value });
  return NextResponse.json({ ok: true });
}
