...
describe('applyLoad', () => {
  it('given the current generation, should replace messages with the loaded set', () => {
    const initial: ConversationMessagesById = {
      c1: { messages: [msg('old')], optimisticSends: [], loadGeneration: 1, pendingMutationsSinceLoad: [], loadStatus: 'loaded' },
    };
  ...
  it('given a stale generation (a newer load has since started), should no-op and return the same reference', () => {
    const initial: ConversationMessagesById = {
      c1: { messages: [msg('old')], optimisticSends: [], loadGeneration: 2, pendingMutationsSinceLoad: [], loadStatus: 'loaded' },
    };
  ...
  it('given a fresh load, should preserve the loadGeneration value', () => {
    const initial: ConversationMessagesById = {
      c1: { messages: [], optimisticSends: [], loadGeneration: 5, pendingMutationsSinceLoad: [], loadStatus: 'loaded' },
    };
  ...
  it('given other conversations tracked, should not touch them', () => {
    const initial: ConversationMessagesById = {
      ...
      other: { messages: [msg('x')], optimisticSends: [], loadGeneration: 9, pendingMutationsSinceLoad: [], loadStatus: 'loaded' },
    };
