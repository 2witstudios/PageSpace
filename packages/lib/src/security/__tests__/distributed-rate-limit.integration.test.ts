...
describe('distributed-rate-limit integration (Postgres)', () => {
  ...
  it('sliding window: same bucket stays blocked, next bucket allows', async () => {
    if (!dbAvailable) return;
    ...
    const r1 = await checkDistributedRateLimit(key, cfg); // t
    expect(r1.allowed).toBe(true);

    const r2 = await checkDistributedRateLimit(key, cfg); // t + ~0s — same bucket
    expect(r2.allowed).toBe(false);

    // Cross the bucket boundary.
    await new Promise((r) => setTimeout(r, windowMs));

    const r3 = await checkDistributedRateLimit(key, cfg); // new bucket
