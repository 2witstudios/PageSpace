...
describe('buildSummarizationPrompt', () => {
  ...
  it('renders normalized tool-call and tool-result parts', () => {
    const { prompt } = buildSummarizationPrompt({
      previousSummary: null,
      transcript: [
        {
          role: 'assistant',
          parts: [
            { type: 'tool-call', toolName: 'read_page', args: { pageId: 'abc' } },
            { type: 'tool-result', toolName: 'read_page', result: 'page content here' },
          ],
        },
      ],
      maxSummaryTokens: 2000,
