...
async function deriveKey(salt) {
    try {
        return (await scryptAsync(getEncryptionKey(), salt, KEY_LENGTH));
    }
    catch (error) {
...
async function decryptLegacy(encryptedText) {
    try {
        const parts = encryptedText.split(':');
        ...
        const key = (await scryptAsync(getEncryptionKey(), legacySalt, KEY_LENGTH));
...
async function decryptNew(encryptedText) {
    try {
        const parts = encryptedText.split(':');
