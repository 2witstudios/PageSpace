...
export async function isUserDriveMember(
  ...
  return membership.length > 0 && !isGuestRole(membership[0].role);
}
...
export async function getUserAccessiblePagesInDrive(
  ...
  const memberCheck = await db.select({ id: driveMembers.id, role: driveMembers.role, customRoleId: driveMembers.customRoleId })
    .from(driveMembers)
    .where(and(
      eq(driveMembers.driveId, driveId),
      eq(driveMembers.userId, userId),
  ...
  // A GUEST sees only its explicit grants (below): no rule 4, no custom role.
  const memberRow = memberCheck[0] && !isGuestRole(memberCheck[0].role) ? memberCheck[0] : null;
...
export async function getUserAccessiblePagesInDriveWithDetails(
  ...
  const memberCustomRoleId = memberRow?.customRoleId ?? null;
...
export async function getUserDriveAccess(
  ...
  try {
    ...
    const membership = await db.select({ id: driveMembers.id, role: driveMembers.role })
      .from(driveMembers)
      .where(and(
    ...
    // A GUEST has drive access only while it still holds a page grant (below).
    if (membership.length > 0 && !isGuestRole(membership[0].role)) {
      if (!silent) {
...
export async function getUserDrivePermissions(
  ...
  try {
    ...
    // A GUEST is a page collaborator, not a member: no drive-wide permissions.
    if (membership.length > 0 && !isGuestRole(membership[0].role)) {
