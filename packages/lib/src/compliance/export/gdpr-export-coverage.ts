...
export const EXCLUDED_TABLES: Readonly<Record<string, string>> = {
  ...
  ...withReason(
    ...
    'drive_backups',
    'drive_roles',
    // A persistent per-drive MACHINE (dev/staging/deploy). The row is the
    // drive's infrastructure, not the subject's data: a name, a kind, and
    // pointers at a VM. `createdBy` is audit-only and confers nothing — the box
    ...
    // rationale describes. What the subject actually authored INSIDE a box is
