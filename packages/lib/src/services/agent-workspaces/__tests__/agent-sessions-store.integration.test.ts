/**
 * `agent_workspaces` store — REAL Postgres.
 *
 ...
 * Run with:
 *     bun run --filter '@pagespace/lib' test:db -- src/services/agent-workspaces/__tests__/agent-sessions-store.integration.test.ts
 */
 ...
 *
 * The AFTER-DELETE trigger on `agent_workspaces` enqueues a reclaim row for each
...
afterAll(async () => {
  ...
  if (conversationIds.length > 0) {
    ...
    // agent_workspaces cascades from conversations, but delete explicitly: the DB
    // is shared, so a leaked row is the next run's mystery, not this one's.
    await db.delete(agentWorkspaces).where(inArray(agentWorkspaces.id, workspaceIds));
...
describe('updateSpriteIdentity — CAS on the previous pointer', () => {
  it('given the expected NULL pointer, should persist identity', async () => {
    ...
    expect(ok).toBe(true);
    const [row] = await db.select().from(agentWorkspaces).where(eq(agentWorkspaces.id, conversationId));
...
describe('recordStorageMeasurement', () => {
  it('given a live row, should persist the measurement', async () => {
    ...
    await store.recordStorageMeasurement({ workspaceId, spriteInstanceId: 'i', measuredBytes: 4096, measuredAt });

    const [row] = await db.select().from(agentWorkspaces).where(eq(agentWorkspaces.id, workspaceId));
  ...
  it('given the Sprite generation moved on mid-measurement, should DROP the stale write', async () => {
    ...
    const workspaceId = await seedSession({ sandboxId: 'sbx-stable-name', spriteInstanceId: 'instance-A' });
  ...
  it('given the same generation still on the row, should persist despite an identical sandbox NAME', async () => {
    ...
    // that was measured.
    const workspaceId = await seedSession({ sandboxId: 'sbx-stable-name', spriteInstanceId: 'instance-B' });
  ...
  it('given a driver that reports no instance id, should still persist against a null row', async () => {
    ...
    const workspaceId = await seedSession({ sandboxId: 'sbx-no-instance', spriteInstanceId: null });
...
describe('reclaim trigger — every cascade path that can strand a live Sprite', () => {
  ...
  it('given the AGENT PAGE is deleted, should NOT touch the session — sessions are not page-anchored', async () => {
    ...
    const rows = await db.select().from(agentWorkspaces).where(eq(agentWorkspaces.id, probe.probeSessionId));
    expect(rows).toHaveLength(1);
...
describe('list bounds and ordering (review M3/M4)', () => {
  ...
  it('orders newest activity first, nulls last — a poll must not reshuffle between refreshes', async () => {
    ...
    await db.update(agentWorkspaces).set({ lastActiveAt: new Date(Date.now() - 60_000) }).where(eq(agentWorkspaces.id, older));
    await db.update(agentWorkspaces).set({ lastActiveAt: new Date() }).where(eq(agentWorkspaces.id, newer));
    await db.update(agentWorkspaces).set({ lastActiveAt: null }).where(eq(agentWorkspaces.id, never));
...
describe('list — driveId filter also includes the caller\'s own global sessions', () => {
  it('includes a null-driveId (global-assistant) session for the same owner', async () => {
    ...
    workspaceIds.push(globalSessionId);
    await db.insert(agentWorkspaces).values({
      id: globalSessionId,
      ownerId,
      driveId: null,
  ...
  it('still excludes a null-driveId session belonging to a DIFFERENT owner', async () => {
    ...
    try {
      ...
      await db.delete(agentWorkspaces).where(eq(agentWorkspaces.id, otherGlobalSessionId));
...
describe('createIfUnderLimit — the spawn ceiling made atomic (review #2261/2)', () => {
  it('given the owner is under the ceiling, should mint a session', async () => {
    ...
    if (!result.ok) return;
    workspaceIds.push(result.session.id);
  ...
  it('given N concurrent spawns racing the SAME ceiling, should mint EXACTLY the remaining allowance — not N (the TOCTOU a count-then-insert pre-check left open)', async () => {
    ...
    for (const r of minted) workspaceIds.push(r.session.id);
