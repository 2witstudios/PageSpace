...
function deps(over: Partial<PreviewAccessDeps> & { calls?: string[] } = {}): PreviewAccessDeps & { calls: string[] } {
  ...
  return {
    ...
    previewStore: { findByHolder: async () => track('findRow', liveRow()), upsert: async () => {} },
    featureEnabled: () => true,
    now: () => NOW,
    ...over,
  };
...
describe('resolvePreviewTarget — the gather, in order', () => {
  ...
  it('no row, no sprite, or a vanished sprite is no-preview', async () => {
    assert({ given: 'no row', should: 'no-preview', actual: (await resolvePreviewTarget({ holder, userId: USER, deps: deps({ previewStore: { findByHolder: async () => null, upsert: async () => {} } }) })).decision.kind === 'refuse' && 'no-preview', expected: 'no-preview' });
