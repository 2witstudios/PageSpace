...
function deps(over: Partial<PreviewAccessDeps> & { calls?: string[] } = {}): PreviewAccessDeps & { calls: string[] } {
  ...
  return {
    ...
    selectPort: async () => null, findStoppedWithRelay: async () => [], markSwept: async () => {}, markRelayStopped: async () => {} },
    featureEnabled: () => true,
    now: () => NOW,
    ...over,
  };
...
describe('resolvePreviewTarget — the gather, in order', () => {
  ...
  it('no row, no sprite, or a vanished sprite is no-preview', async () => {
    assert({ given: 'no row', should: 'no-preview', actual: (await resolvePreviewTarget({ holder, userId: USER, sessionId: SESSION, deps: deps({ previewStore: { findByHolder: async () => null, upsert: async () => true, setStoppedByUser: async () => null, approvePort: async () => null,
    selectPort: async () => null, findStoppedWithRelay: async () => [], markSwept: async () => {}, markRelayStopped: async () => {} } }) })).decision.kind === 'refuse' && 'no-preview', expected: 'no-preview' });
