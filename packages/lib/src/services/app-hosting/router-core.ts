...
export type AppRouteDecision =
  ...
  | { kind: 'parked'; reason: 'out_of_credits' | 'parked_status' | 'daily_cap'; driveId?: string }
  | { kind: 'unavailable'; reason: 'deploying' | 'failed' | 'destroying' | 'hosting_disabled'; driveId?: string }
...
export function decideAppRoute(input: AppRouteInput): AppRouteDecision {
  ...
  if (app.status === 'parked') return { kind: 'parked', reason: 'parked_status', driveId: app.driveId };
  if (app.status === 'destroying') return { kind: 'unavailable', reason: 'destroying', driveId: app.driveId };
  if (app.status === 'failed') return { kind: 'unavailable', reason: 'failed', driveId: app.driveId };
  ...
  if (!SERVABLE_STATUSES.has(app.status)) return { kind: 'unavailable', reason: 'deploying', driveId: app.driveId };
  if (!app.hasMachine) return { kind: 'unavailable', reason: 'deploying', driveId: app.driveId };

  if (app.tier === 'metered' && !input.balanceOk) {
    return { kind: 'parked', reason: 'out_of_credits', driveId: app.driveId };
  }
