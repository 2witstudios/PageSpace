...
const mockDb = vi.hoisted(() => {
  ...
  return {
    ...
    transaction: async (cb: (tx: unknown) => Promise<unknown>) => cb({ select, update }),
  };
});
...
describe('syncAppTierToSubscription', () => {
  it('acks a subscription nothing local points at', async () => {
    ...
    mockDb.__state.rows = [];
    const outcome = await syncAppTierToSubscription('sub_unknown', 'active');
    assert({
      given: 'a subscription with no mirror row',
