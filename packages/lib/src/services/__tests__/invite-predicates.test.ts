...
describe('isInviteExpired', () => {
  ...
  it('given now equals expiresAt, should return true (boundary)', () => {
    expect(
      isInviteExpired({ expiresAt: new Date(now.getTime()), now })
    ).toBe(true);
  });

  it('given now is one millisecond before expiresAt, should return false', () => {
    expect(
      isInviteExpired({ expiresAt: new Date(now.getTime() + 1), now })
    ).toBe(false);
  });
...
describe('isEmailMatchingInvite', () => {
  ...
  it('given empty strings on both sides, should return true', () => {
    expect(
      isEmailMatchingInvite({ inviteEmail: '', userEmail: '' })
    ).toBe(true);
  });
});
