 ...
 * 1-indexed to match `form_targets.headerRow`. `fields[i]` always maps to
 * column `i`, fixed at wire time — a form's field list is never mutated
 * afterward (see docs/specs/canvas-forms.md), so there's no reordering or
 * skipping to account for here.
 */
