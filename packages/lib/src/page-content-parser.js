...
exports.getPageContentForAI = getPageContentForAI;
 ...
 */
function convertContentToPlainText(content) {
 ...
 */
function getPageContentForAI(page) {
    if (!page) {
    ...
    switch (page.type) {
        ...
        case enums_1.PageType.CHANNEL:
            if (page.channelMessages && page.channelMessages.length > 0) {
                ...
                page.channelMessages.forEach((msg) => {
        ...
        case enums_1.PageType.SHEET: {
            try {
                ...
                if (typeof page.content === 'string' && page.content.trim().startsWith('#%PAGESPACE_SHEETDOC')) {
                    ...
                    const sheetData = (0, sheet_1.parseSheetContent)(page.content);
                    const evaluation = (0, sheet_1.evaluateSheet)(sheetData);
                    ...
                    const columnHeaders = Array.from({ length: maxCols }, (_, index) => (0, sheet_1.encodeCellAddress)(0, index).replace(/\d+/g, ''));
