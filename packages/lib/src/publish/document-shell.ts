...
/**
 * Content-Security-Policy for published non-canvas pages (documents, code,
 * sheets, …). Unlike the canvas baseline (`../canvas/csp.ts`), which allows
 * `script-src 'unsafe-inline'` because canvas isolation is by origin, these
 * page types never execute author script — `script-src 'none'` is the hard
 * enforcement backing the markup-level hygiene each renderer already applies
 * (e.g. `sanitize-document-html.ts` for documents; code/sheet renderers emit
 * no script-bearing markup in the first place).
 */
export function buildDocumentCsp(): string {
  return "default-src 'none'; img-src data: https:; style-src 'unsafe-inline'; script-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'";
}

/**
 * Assemble a complete, standalone HTML document around already-built body
 * markup (typically the output of `wrapDocumentBody`). Shared head-assembly
 * for every publishable page type that is NOT canvas (documents, code,
 * sheets, …) — each renderer builds its own `bodyHtml` and delegates the
 * doctype/head/CSP/typography wrapper to this one function so they stay
...
export function renderDocumentShell({
  ...
  return (
    ...
    `<meta http-equiv="Content-Security-Policy" content="${buildDocumentCsp()}">` +
    `<style>${DOCUMENT_TYPOGRAPHY_CSS}</style>` +
    '</head><body>' +
    bodyHtml +
    '</body></html>'
  );
