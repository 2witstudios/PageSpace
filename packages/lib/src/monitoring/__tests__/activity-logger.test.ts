...
/**
 * Wait for the fire-and-forget logActivity chain to settle.
 * logActivity has multiple awaited steps (getLatestLogHash → db.insert().values()),
...
describe('activity-logger', () => {
  ...
  describe('logActivityWithTx', () => {
    ...
    it('should return a deferred workflow trigger when workflow hook is set', async () => {
      ...
      deferred!();
      await vi.waitFor(() => { expect(workflowHook).toHaveBeenCalledTimes(1); });
