 ...
 * agent writes can change what the human reads. `bodyBytes` comes from the
...
export const renderApprovalSubject: RenderApprovalSubject = ({ canonical }) => {
  ...
  return {
    headline: `${canonical.method} ${canonical.origin}${canonical.path} — ${canonical.operation.name} (${canonical.operation.class})`,
