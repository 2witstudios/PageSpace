/**
 * ADR 0005 §10.21 (G1a review H2) — TYPE-LEVEL: `rebind` is not callable with an identity whose
 * `channel` is not `'manage'` (G1c E3 renamed the field from `audience`). `tsc` over this file is the assertion (an unused `@ts-expect-error` is
...
export async function rebindNeedsAManageIdentity(adapter: StoreAdapter, identity: StoreIdentity, input: RebindInput) {
  ...
  // @ts-expect-error — a StoreIdentity whose channel is not 'manage' cannot rebind
