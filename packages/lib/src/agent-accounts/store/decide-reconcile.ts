 ...
 * version and its digest. Anything else — an older version (the write never
 * landed), a newer one, another writer's bytes at the pending version, or an
 * Infisical that cannot be read — is `fail_closed`: nothing is served or
 * written and the ref stays reconcile-required. Digests go through the
 * timing-safe compare. Pure.
 */
import { secureCompare } from '../../auth/secure-compare';
import type { DecideReconcile } from './store-adapter';

export const decideReconcile: DecideReconcile = ({ pending, observed }) => {
