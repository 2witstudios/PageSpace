...
describe('adversarial: owner-origin-tampering', () => {
  ...
  it.todo('given a widening performed through rebind with an OwnerConsent to exactly the new bindings, should rewrite bindings and a grant signed over them should resolve [0005 §10.21; G1a review H2]');
  it.todo('given a DB writer who reassigned the owner and then produced a consent from the NEW owner, should return consent_invalid (the plane checks the STORED ownerRef) [0005 F15]');
  it.todo('given a policyVersion bump written through rebind, should refuse an outstanding grant signed over the old bindings with binding_mismatch at resolve [0005 §10.21]');
});
