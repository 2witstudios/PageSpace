...
function row(overrides: Partial<SpriteHolderLifecycleRow> = {}): SpriteHolderLifecycleRow {
  return {
    holderId: 'conv-1',
...
describe('planSpriteHolderLifecycle — ensure (lazy first touch)', () => {
  it('given no row, should create', () => {
    const plan = planSpriteHolderLifecycle({ row: null, intent: 'ensure', canRun: true, now: NOW });
  ...
  it('given an unprovisioned row, should create (the sandbox is acquired lazily, not at row insert)', () => {
    const plan = planSpriteHolderLifecycle({ row: unprovisioned, intent: 'ensure', canRun: true, now: NOW });
    expect(plan.action).toBe('create');
  });

  it('given a torn-down row, should create and REVIVE the row (same key, fresh Sprite)', () => {
    const plan = planSpriteHolderLifecycle({ row: tornDown, intent: 'ensure', canRun: true, now: NOW });
    expect(plan.action).toBe('create');
  ...
  it('given a provisioned row, should resume under the SAME key', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'ensure', canRun: true, now: NOW });
  ...
  it('given canRun false, should deny BEFORE handing back any warm session', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'ensure', canRun: false, now: NOW });
  ...
  it('given canRun false and no row, should deny rather than create a billable VM', () => {
    const plan = planSpriteHolderLifecycle({ row: null, intent: 'ensure', canRun: false, now: NOW });
    expect(plan.action).toBe('deny');
  });
});

describe('planSpriteHolderLifecycle — attach (read-only resolve; never mints a VM)', () => {
  it('given a provisioned row, should resume', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'attach', canRun: true, now: NOW });
    expect(plan.action).toBe('resume');
  });

  it('given no row, should deny (a PTY connect addresses an EXISTING session)', () => {
    const plan = planSpriteHolderLifecycle({ row: null, intent: 'attach', canRun: true, now: NOW });
    if (plan.action !== 'deny') throw new Error('expected deny');
    expect(plan.reason).toBe('session_not_found');
  });

  it('given a torn-down row, should deny rather than share or resurrect a Sprite', () => {
    const plan = planSpriteHolderLifecycle({ row: tornDown, intent: 'attach', canRun: true, now: NOW });
    if (plan.action !== 'deny') throw new Error('expected deny');
    expect(plan.reason).toBe('session_torn_down');
  });

  it('given an unprovisioned row, should deny — attach never provisions (ensure does)', () => {
    const plan = planSpriteHolderLifecycle({ row: unprovisioned, intent: 'attach', canRun: true, now: NOW });
    if (plan.action !== 'deny') throw new Error('expected deny');
    expect(plan.reason).toBe('sandbox_not_provisioned');
  });

  it('given canRun false, should deny (attach re-authorizes every time)', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'attach', canRun: false, now: NOW });
    if (plan.action !== 'deny') throw new Error('expected deny');
    expect(plan.reason).toBe('not_authorized');
  });
});
...
describe('planSpriteHolderLifecycle — live-instance reconciliation (ABA)', () => {
  ...
  it('given an adopt, should drop the stored storage measurement (a new VM has a fresh disk)', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'ensure', canRun: true, now: NOW, liveInstance: live });
    expect(plan.stamps).toMatchObject({
      lastActiveAt: NOW,
      teardownRequestedAt: null,
  ...
  it('given canRun false, should deny before reconciling anything', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'ensure', canRun: false, now: NOW, liveInstance: live });
    expect(plan.action).toBe('deny');
  });
});

describe('planSpriteHolderLifecycle — end (instance-guarded teardown, row retained)', () => {
  it('given a provisioned row, should tear down guarded by the recorded INSTANCE', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'end', canRun: true, now: NOW });
  ...
  it('given canRun false, should STILL tear down (cleanup is never gated on authorization)', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'end', canRun: false, now: NOW });
    expect(plan.action).toBe('teardown');
  });

  it('given no row, should noop', () => {
    const plan = planSpriteHolderLifecycle({ row: null, intent: 'end', canRun: true, now: NOW });
  ...
  it('given an unprovisioned row, should noop but still mark the session ended', () => {
    const plan = planSpriteHolderLifecycle({ row: unprovisioned, intent: 'end', canRun: true, now: NOW });
    if (plan.action !== 'noop') throw new Error('expected noop');
  ...
  it('given an already torn-down row, should noop with nothing left to stamp', () => {
    const plan = planSpriteHolderLifecycle({ row: tornDown, intent: 'end', canRun: true, now: NOW });
    if (plan.action !== 'noop') throw new Error('expected noop');
  ...
  it('given a NORMALLY-ended row (provisioned, CONFIRMED kill — the common shape), should noop rather than re-teardown', () => {
    ...
    const plan = planSpriteHolderLifecycle({ row: endedProvisioned, intent: 'end', canRun: true, now: NOW });
    if (plan.action !== 'noop') throw new Error('expected noop');
    expect(plan.reason).toBe('already_ended');
    expect(plan.stamps).toEqual({});
  });
  ...
  it('a REOPENED row (endedAt withdrawn by planSessionReopen, kill still confirmed) is re-endable: the fresh end-intent is stamped, nothing is re-killed', () => {
    ...
    const plan = planSpriteHolderLifecycle({ row: reopened, intent: 'end', canRun: true, now: NOW });
    if (plan.action !== 'noop') throw new Error('expected noop');
    expect(plan.stamps.endedAt).toEqual(NOW);
  });
});
...
describe('planSpriteHolderLifecycle — reprovision (heal a row whose Sprite is unusable)', () => {
  it('given a provisioned row, should create while CASing against the old pointer', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'reprovision', canRun: true, now: NOW });
    if (plan.action !== 'create') throw new Error('expected create');
    expect(plan.previousSandboxId).toBe('pgs-ses-abc');
  });

  it('given an unprovisioned row, should create', () => {
    const plan = planSpriteHolderLifecycle({ row: unprovisioned, intent: 'reprovision', canRun: true, now: NOW });
    expect(plan.action).toBe('create');
  });

  it('given a torn-down row, should create and revive it', () => {
    const plan = planSpriteHolderLifecycle({ row: tornDown, intent: 'reprovision', canRun: true, now: NOW });
    expect(plan.action).toBe('create');
  ...
  it('given no row, should deny (there is nothing to reprovision)', () => {
    const plan = planSpriteHolderLifecycle({ row: null, intent: 'reprovision', canRun: true, now: NOW });
    if (plan.action !== 'deny') throw new Error('expected deny');
    expect(plan.reason).toBe('session_not_found');
  });

  it('given canRun false, should deny', () => {
    const plan = planSpriteHolderLifecycle({ row: row(), intent: 'reprovision', canRun: false, now: NOW });
    expect(plan.action).toBe('deny');
  });
});
  ...
  it('should never emit a teardown for any intent other than end', () => {
    ...
    for (const intent of intents) {
      for (const candidate of rows) {
        const plan = planSpriteHolderLifecycle({ row: candidate, intent, canRun: true, now: NOW });
    ...
    for (const intent of intents) {
      for (const candidate of [null, row(), unprovisioned, tornDown]) {
        for (const canRun of [true, false]) {
          const plan = planSpriteHolderLifecycle({ row: candidate, intent, canRun, now: NOW });
