 ...
 * spend (ADR 0007 Decision 8, "owner pays").
 *
 ...
 * `releaseHold` — and injects the `agent_identities.ownerUserId` lookup; the
...
export interface ResolveBillingPayerInput {
  ...
  userId: string;
  /** `agent_identities.ownerUserId` for that principal — null/empty when unclaimed or not an agent. */
