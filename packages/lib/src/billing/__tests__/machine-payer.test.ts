...
import { resolveMachinePayerId, resolveAgentSessionPayerId, lookupPageOwnerId } from '../machine-payer';

describe('resolveMachinePayerId', () => {
  it('falls back to tenantId when there is no backing machine page (e.g. an own machine with no active page yet)', async () => {
    const lookup = vi.fn();
    await expect(
      resolveMachinePayerId({ tenantId: 'owner-1', lookupPageOwnerId: lookup }),
  ...
  it('is not a passthrough — a resolved owner beats a different tenantId', async () => {
    ...
    const a = await resolveMachinePayerId({ tenantId: 'a', machinePageId: 'p', lookupPageOwnerId: lookup });
    const b = await resolveMachinePayerId({ tenantId: 'b', machinePageId: 'p', lookupPageOwnerId: lookup });
