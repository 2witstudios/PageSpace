...
describe('MON-5 one definition of a credit', () => {
  ...
  it('MON-5 centsFromCredits returns an exact integer for every whole credit count 0..10,000', () => {
    ...
    // (an independent value, not the implementation's formula restated).
    expect(CREDITS_PER_DOLLAR).toBe(100);
    for (let credits = 0; credits <= 10_000; credits++) {
      const cents = centsFromCredits(credits);
      if (!Number.isInteger(cents) || cents !== credits) {
  ...
  it('MON-5 creditsFromCents returns an exact integer for every whole cent amount 0..10,000', () => {
    ...
    for (let cents = 0; cents <= 10_000; cents++) {
      ...
      if (!Number.isInteger(credits) || credits !== cents) {
  ...
  it('MON-5 (partial) providerDollarsCoveredByCents takes charged cents back through the markup to the provider cost they cover', () => {
    ...
    // Multiply before divide. The divide-first form this replaces (cents / 100 / (markup / 10000))
    // gives 0.09999999999999999, 0.11333333333333334, 0.13999999999999999, 0.15333333333333335
    // and 0.18000000000000002 for these inputs; literal expectations, never the formula itself.
    expect(providerDollarsCoveredByCents(15)).toBe(0.1);
    expect(providerDollarsCoveredByCents(17)).toBe(0.11333333333333333);
