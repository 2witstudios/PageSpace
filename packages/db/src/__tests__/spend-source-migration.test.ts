/**
 * Migration 0320 — where the spend source is stored (Spec SPEND-3).
 *
...
const file = readdirSync(MIGRATIONS_DIR).find((f) => /^0320_.*\.sql$/.test(f));
const sql = file ? readFileSync(path.join(MIGRATIONS_DIR, file), 'utf8') : '';
const statements = sql.split('--> statement-breakpoint').map((s) => s.trim()).filter(Boolean);

describe('drizzle/0320 — the stored spend source (static)', () => {
...
describeLive('drizzle/0320 against a real Postgres', () => {
  let pool: Pool;
