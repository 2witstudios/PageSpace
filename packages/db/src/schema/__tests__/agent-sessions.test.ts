 ...
 *  - **The session owns the sandbox, not the shell** — `agent_session_shells`
...
const shellsConfig = getTableConfig(agentSessionShells);
const shellsColumns = getTableColumns(agentSessionShells);
...
describe('agent_sessions schema — Sprite identity and storage', () => {
  it.each([
    'sessionKey',
    'sandboxId',
    'spriteInstanceId',
  ...
  it('given the sprite key is never a lookup address, sessionKey should not be unique', () => {
    expect(sessionsColumns.sessionKey.isUnique).toBeFalsy();
  });
...
describe('agent_session_shells schema', () => {
  ...
  it('given tab titles must be unambiguous, (sessionId, name) should be unique — per session, never globally', () => {
    const unique = shellsConfig.indexes.find(
      (index) => index.config.name === 'agent_session_shells_session_name_idx'
    );
  ...
  it('indexes sessionId for the per-session shell list', () => {
    const indexNames = shellsConfig.indexes.map((index) => index.config.name);
    expect(indexNames).toContain('agent_session_shells_session_id_idx');
  });
  ...
  it('given the SESSION owns the sandbox, a shell should carry NO Sprite or storage columns', () => {
    ...
    for (const columnName of forbidden) {
      expect(columnName in shellsColumns, `${columnName} must not exist on agent_session_shells`).toBe(false);
    }
  });

  it('given a PTY row can predate its first connect, command and streamSessionId should be nullable', () => {
    ...
    expect(shellsColumns.streamSessionId.notNull).toBe(false);
  });
...
describe('agent sessions relations', () => {
  it('exports both tables relations, wiring a session to its shells', () => {
    expect(agentSessionsRelations).toBeDefined();
    expect(agentSessionShellsRelations).toBeDefined();
  });
});
...
describe('agent_sessions sprite reclaim trigger (0238)', () => {
  ...
  it('given a shell has no Sprite pointer, defines no trigger on agent_session_shells', () => {
    expect(triggerSql).not.toMatch(/ON agent_session_shells/);
  });
});
