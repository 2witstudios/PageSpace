...
describe('schema barrel enumerates the org tables', () => {
  it('ORG-1 ORG-2 ORG-3 registers organizations, org_members and org_invitations in the combined schema', () => {
...
describe('organizations', () => {
  ...
  it('ORG-1 carries name, a globally unique slug, an avatar and a required owner', () => {
  ...
  it('ORG-1 restricts deleting the owning user (ORG-6: an Owner cannot delete their account while owning an org)', () => {
    expect(foreignKeyOn(organizations, 'ownerId')).toEqual({ onDelete: 'restrict', target: 'users' });
  });

  it('ORG-1 holds org policies as NOT NULL jsonb defaulting to an empty object', () => {
  ...
  it('ORG-1 keeps the Stripe fields nullable (an org exists before it pays)', () => {
...
describe('org_members', () => {
  ...
  it('ORG-2 exports the closed OrgRole set Owner, Admin, Member', () => {
  ...
  it('ORG-2 allows a user in many orgs but only once per org', () => {
...
describe('org_invitations', () => {
  ...
  it('ORG-3 stores only a token hash, never the raw token, plus email, role, expiry and acceptance', () => {
...
describe('drives org columns', () => {
  ...
  it('DRV-1 a drive can be owned by an org; deleting an org with drives is refused (ORG-6), never set null', () => {
    expect(columns.orgId.notNull).toBe(false);
    expect(foreignKeyOn(drives, 'orgId')).toEqual({ onDelete: 'restrict', target: 'organizations' });
  });

  it('DRV-1 Home drives never can: a CHECK refuses orgId on a HOME drive', () => {
  ...
  it('DRV-1 drive slugs are unique per org for org drives (D-OW-15)', () => {
...
describe('drive_members.source', () => {
  ...
  it('DRV-8 distinguishes invited members (guests included) from org-materialized rows, defaulting to invite (D-OW-6)', () => {
