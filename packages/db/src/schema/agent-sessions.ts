 ...
 * `conversations.sessionId` FKs here, set at creation and permanent — a
 ...
 * tab titles only — see `agent_session_shells`.)
 *
 ...
 * `shellId` and nothing else. `sessionId` FKs the owning session, cascading so
...
export const agentSessionShells = pgTable('agent_session_shells', {
  ...
  /** The owning session (`agent_sessions.id`). */
