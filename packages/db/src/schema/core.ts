...
export const pages = pgTable('pages', {
  ...
  toolApprovalMode: text('toolApprovalMode', { enum: ['ask', 'auto'] }).default('ask').notNull(), // AI_CHAT agents: whether gated writes pause for the user's approval ('ask', the default) or run unprompted ('auto'). Only interactive turns can pause — a turn with no browser session (workflow, trigger, channel mention) or a dispatched worker turn runs as 'auto' regardless.
