...
-- inserted. Its replacement, `content_tags` (0268), covers the same ground and
...
--
-- SEPARATE FILE from 0268 on purpose. `packages/db/src/migration-sql-analysis.ts`
-- sets `singleDoBlock: false` when a second `DO $$` appears ANYWHERE in a file,
-- and `allDropsInsideDoBlock` is derived from it — so 0268's guarded pre-step
