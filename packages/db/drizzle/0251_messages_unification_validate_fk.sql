...
-- 0249's `NOT VALID` foreign key into a validated one, so
...
-- below is hand-written (repo convention — precedent 0249/0247/0225/0222/0116:
...
--      nothing, and a database that has never seen 0249 is not silently
--      "validated" by omission.
--   2. PRE-VALIDATE AUDIT — the two things 0249 left behind, handled BEFORE
--      the VALIDATE rather than discovered as an opaque 23503 inside it:
--        a. the orphan conversations 0249 SKIPPED (it RAISEd WARNING with a
--           count). Those rows still violate the FK → FAIL LOUDLY, naming them.
--        b. the existing `conversations` rows that disagree with their
--           `chat_messages` about the page (0249 RAISEd NOTICE). They do NOT
...
BEGIN
  ...
  IF fk_exists IS NOT TRUE THEN
    -- 0249 adds it unconditionally, so this can only mean the constraint was
    ...
    RAISE EXCEPTION 'messages unification (0251): constraint chat_messages_conversationId_conversations_id_fk does not exist. Migration 0249 adds it; it must not be dropped. Re-add it exactly as 0249 does (FOREIGN KEY ("conversationId") REFERENCES "conversations"("id") ON DELETE CASCADE, NOT VALID) and re-run.';
  END IF;

  IF fk_validated THEN
    RAISE NOTICE 'messages unification (0251): FK already validated — nothing to do.';
    RETURN;
  END IF;

  -- ── 2a. THE BLOCKER: rows 0249 could not give a parent ──────────────────
  -- 0249's synthesis ladder is (earliest user-role author → page creator →
  ...
  IF orphan_conversations > 0 THEN
    ...
    RAISE EXCEPTION E'messages unification (0251) cannot validate the chat_messages FK: % conversation id(s), covering % message row(s), still have no "conversations" row. These are migration 0249''s SKIPPED orphans (it raised a WARNING with the same count).\nFirst (up to 50): %\nRepair, then re-run. Either (a) INSERT a "conversations" row for each — type=''page'', contextId = the messages'' "pageId", userId = a deliberately chosen owner (0249''s ladder is: earliest user-role message author, else pages."createdBy", else drives."ownerId"), isActive = false if every message is already soft-deleted; or (b) DELETE the message rows if they are known garbage. This migration will not choose for you: option (a) hands someone a chat history and option (b) destroys one.',
      orphan_conversations, orphan_rows, orphan_sample;
  END IF;

  -- ── 2b. THE NON-BLOCKER: conversations that disagree about the page ─────
  -- 0249 reported these as a NOTICE and did not repair them. They satisfy the
