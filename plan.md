...
- [OpenAI-Compatible Inference API](tasks/openai-compatible-inference-api.md) — `POST /api/v1/chat/completions` exposing PageSpace agents as OpenAI "models" (`ps-agent://<pageId>`) authenticated by MCP tokens as API keys; pure request/response adapters compose the existing chat pipeline. Combines GitHub issues #1360 + #1361.
