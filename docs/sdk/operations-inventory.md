...
| `reorder_task` (tools.js:504) | `pageId*`, `taskId*`, `position*` | PATCH `/api/pages/{pageId}/tasks/{taskId}` `{position}` (task.js:207) | same PATCH route (`:24`, `position` accepted at `:64`) | Updated task row. (The web UI's separate `pages/[pageId]/tasks/reorder/route.ts` exists but the MCP path uses the PATCH — both are valid; SDK should standardize on PATCH `{position}`.) |
