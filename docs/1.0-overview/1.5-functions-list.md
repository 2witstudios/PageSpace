...
**Dependencies:** [`createHmac`](crypto:createHmac()), [`secureCompare`](packages/lib/src/auth/secure-compare.ts).
**Last Updated:** 2025-08-21
...
#### update_task
**Purpose:** Add or update tasks on TASK_LIST pages. Works with pages created via create_page(type: TASK_LIST). Each task automatically creates a linked DOCUMENT page for detailed notes.
**Parameters:** taskId (optional, for updates), pageId (required for new tasks), title, description, priority, status
**Dependencies:** [`db`](packages/db), [`broadcastTaskEvent`](apps/web/lib/socket-utils).
**Last Updated:** 2025-12-01

**Note:** Task lists are created using `create_page` with `type: 'TASK_LIST'`. Use `read_page` on TASK_LIST pages to get structured task data with progress statistics.
...
#### create_page
**Purpose:** Create new pages with support for all page types including FOLDER, DOCUMENT, AI_CHAT, CHANNEL, CANVAS, SHEET, and TASK_LIST. For AI_CHAT pages, use update_agent_config after creation to configure agent behavior.
**Dependencies:** [`db`](packages/db), [`canUserEditPage`](packages/lib/permissions), [`broadcastPageEvent`](apps/web/lib/socket-utils).
**Key Features:**
- **All Page Types**: Supports creation of all page types
- **Simplified Interface**: Basic parameters only (driveId, title, type, parentId)
- **Agent Workflow**: For AI agents, create with type: AI_CHAT, then use update_agent_config
**Last Updated:** 2025-09-10
...
- Use `create_page(type: TASK_LIST)` to create task lists
- Use `read_page` on TASK_LIST pages to view task progress

#### **Agent Management & Communication** (4 functions)
...
- **update_agent_config**: Update agent configuration (create agents with create_page type: AI_CHAT)
