...
#### regex_search
**Purpose:** Search page content using regular expression patterns. Returns pages with pageId, title, and semantic paths for operations and discovery.
**Dependencies:** [`db`](packages/db), [`getUserAccessLevel`](packages/lib/permissions), [`getUserDriveAccess`](packages/lib/permissions).
**Last Updated:** 2025-09-07

#### glob_search
**Purpose:** Find pages using glob-style patterns for titles and paths (e.g., "**/README*", "meeting-*.md"). Returns pageId and semantic path for each match.
**Dependencies:** [`db`](packages/db), [`getUserAccessLevel`](packages/lib/permissions), [`getUserDriveAccess`](packages/lib/permissions).
**Last Updated:** 2025-09-07

#### multi_drive_search
**Purpose:** Search for content across multiple drives simultaneously with automatic permission filtering. Returns results grouped by drive.
...
**Purpose:** Execute multiple page operations atomically (create, move, rename, delete, update) with rollback support.
...
### AI Tools - Agent Management
**Location:** `apps/web/src/lib/ai/tools/agent-tools.ts`
...
The PageSpace codebase contains **over 325 documented functions** across 9 major categories:
...
4. **AI & LLM Functions** (40+ functions): AI integration, enhanced search tools, task management, batch operations, agent creation, and monitoring
...
### **New AI Tool Categories Added (September 2025):**

#### **Enhanced Search & Discovery Tools** (3 functions)
- **regex_search**: Pattern-based content search with permission filtering
...
- **batch_page_operations**: Atomic multi-operation transactions with rollback
...
- **create_page (Enhanced)**: Enhanced page creation with optional agent configuration support
