...
This document provides a comprehensive list of all functions across the PageSpace codebase, organized by category and location. Last updated: 2025-08-28.
...
#### buildSystemPrompt(role: AgentRole, contextType: 'dashboard' | 'drive' | 'page', contextInfo?: { driveName?: string; driveSlug?: string; driveId?: string; pagePath?: string; pageType?: string; breadcrumbs?: string[]; }): string
**Purpose:** Builds a complete system prompt for a specific AI role and context, dynamically including an overview of PageSpace and instructions for context gathering. If a specific page context is available, it prioritizes `read_current_page` or `read_page`. Otherwise, it instructs the AI to use `list_drives` and `list_pages` to gain broader context.
**Dependencies:** [`ROLE_PROMPTS`](apps/web/src/lib/ai/role-prompts:ROLE_PROMPTS), [`buildContextPrompt`](apps/web/src/lib/ai/role-prompts:buildContextPrompt()).
...
The PageSpace codebase contains **over 300 documented functions** across 8 major categories:
...
4. **AI & LLM Functions** (25 functions): AI integration, monitoring, and role management
...
This documentation serves as a complete reference for all functions available in the PageSpace application, enabling developers to quickly understand and extend the codebase.