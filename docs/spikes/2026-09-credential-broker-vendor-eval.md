...
1. Keep a plane metadata row per secret: `(secretRef, version, writerEpoch, leaseExpiresAt)`.
2. The writer takes `SELECT … FOR UPDATE` on that row inside **one Postgres transaction that spans the whole check → Infisical write → commit** (statement and lock timeouts bound the lease). It checks the expected `version`, bumps `writerEpoch`, performs the Infisical write, re-reads `secret.version` from the store, and commits `version+1` in the same transaction. Only one writer can hold the row, so two live writers never interleave.
...
4. **Residual, stated plainly:** a writer paused past its lease can still land a late `PATCH` after a successor committed, because nothing on the store side fences it. The successor's post-commit verification detects the version drift and re-writes the committed value, but between the late write and that repair the store holds a stale value. Only a store-side conditional write, which Infisical lacks, fully prevents this. Every writer must go through the row; any out-of-band writer to the Infisical path is outside the guarantee.

G1b-store (PR #2646, Control Board §1: "version-check-then-write under a per-secret lock") owns the exact schema and transaction boundaries; this spike states only the required properties and the residual. The worker also recovers from a crash mid-refresh (write-then-swap; a row left in `state='writing'` is resolved on restart by reading the store version), revokes, honours RFC 9700, and is the only holder of a refresh-capable identity. G3 already words this as "ours otherwise".
...
5. **Store CAS is ours** (§2.4): the adapter interface G1b freezes should not promise a CAS the backend lacks; name it "versioned write with post-write verification" or implement CAS in the plane's own metadata table. §7.2 gives the required single-writer lease protocol and its residual: a late stale write from a writer past its lease is detected and repaired, not prevented.
