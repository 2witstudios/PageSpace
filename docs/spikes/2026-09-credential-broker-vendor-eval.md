...
# for web/realtime) and never join `credential_plane`; only credential-executor (v1) and the disabled agent-proxy bridge
...
services:
  ...
  vault-ingress:
    ...
    networks: [vault_front, vault_ingress]   # never `traefik`, never `credential_plane` (no route to DB/Redis/agent-proxy)
  ...
  # FUTURE / DISABLED in v1: injects nothing until same-fetch version pinning is verified (§7.2).
  agent-proxy:
...
networks:
  ...
  vault_front:                          # infisical <-> vault-ingress ONLY: a compromised Internet-facing sidecar
    ...
    internal: true                      # its redis, or agent-proxy, which stay on credential_plane)
  vault_ingress:
  ...
  plane_egress:
    driver: bridge                      # NOT internal: `internal` and `credential_plane` are both `internal: true`, so without
                                        ...
                                        # credential-executor (v1) and the disabled agent-proxy are its only members: the plane's single outbound path.
