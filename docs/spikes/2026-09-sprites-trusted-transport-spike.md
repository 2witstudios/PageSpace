...
- *The guest cannot **originate** an authenticated connection* — **verified** (§3.3, and the forged/no-token handshakes above).
...
2. **The guest cannot authenticate itself to anything outside the VM (§3.3).** No metadata endpoint, and the control plane rejects it; the gateway `10.0.0.2` was not port-probed (§3.3), which G4 closes. *Therefore there is no surface on which guest code could present a workload identity the way SPIFFE/SPIRE would* — installing identity machinery inside the same root-controlled guest establishes no boundary, exactly as Codex warned.
...
#    G4 adds: 10.0.0.2:{53,80,443,8080,9000} (gateway — not probed in this run);
