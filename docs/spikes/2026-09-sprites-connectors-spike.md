...
> **Outcome (2026-09-18, later the same day): route DECLINED** — D-34 revised (decision page `o961qhgn9l5ai7g7mkkh7278`, Control Board `pzutwyfnczxwkd61jhk3nya0`). Sprites Connectors will not be the credential plane in any form, including a custom_api-wrapped universal relay: credential-plane coupling to Fly is stickier than compute (OAuth consent accrues; tokens are non-exportable, so demotion = a per-user re-consent campaign), the gateway is physically Fly-only so the provider-agnostic relay must be built regardless, and the custom_api variant re-imports the Fly custody residuals while adding a billed, always-warm relay-sprite failure domain. This spike stands as the rejected-alternative record and exit-cost analysis. Nothing in §7 ("What adoption would have meant for the board — counterfactual") was adopted; the board reverted to the D-18 relay-first design the same afternoon — zero code written, which is the spike-first discipline paying for itself.
> **Epic:** Agent Accounts & Credential Broker (`j471yhv7p3abea7mlxrchdu1`) · **Task:** S4 (this spike) · **Feeds:** Λ1, Λ6, D-24 custody rule, G2/G3/G4 entry, G1c shape re-review. Extends S1 (`2026-09-sprites-trusted-transport-spike.md`), whose "no trusted guest→server channel" conclusion this spike's gateway evidence **partially revises** as a finding; the board kept S1's answer for Λ6 after D-34 (§7.1).
...
4. **Label-crossing fails closed — verified.** Sprite A (label `ps-s4-label-a`) got `GET /user` → `200` with `login: 2witstudios` through connection A and `403 {"error":"sprite is missing required labels: ps-s4-label-b"}` through connection B; sprite B mirrored it exactly (`DaisyDebate` / 403 on A); the control sprite with no labels got 403 through both connections (an unlabeled sprite is denied by a label-requiring policy). The documented default that "a connector with no policy refuses every Sprite" was **not** tested — both connections carried a label policy (**assumed**). Each sprite received **its own** GitHub identity from **its own** connection, and the guest held no token at any point.
5. **Newly labeled sprites match immediately — verified; revocation untested.** A sprite created *after* a connection's policy existed, then labeled via `PUT /v1/sprites/{name}` (a field the SDK types don't know about; the wire accepts it on create and update), could use the connection immediately, with no policy re-save. That shows grants are not frozen at policy-save time; it does **not** show per-request re-evaluation, because no label was removed and re-probed — a gateway that cached authorization on first use would produce the same observation (**assumed** per-request, not verified).
6. **REST yes, git no — verified.** `GET /repos/{owner}/{repo}` through the gateway returns real repo data (200). Git smart HTTP does **not** route: `{conn}/2witstudios/PageSpace.git/info/refs?service=git-upload-pack` returns a GitHub-**REST**-shaped 404 (`docs.github.com/rest` link) — the gateway forwards to `api.github.com`, and git lives on `github.com`. Anonymous `git ls-remote https://github.com/...` from the same sprite worked (public repo). Had the route been adopted, G4's server-side Git relay would still have been needed for git transport, and Connectors would have retired the raw `GH_TOKEN` only for REST and public-repo git.
7. **Refresh: GitHub needs none; the gateway owns it where it exists.** `token_expires_at: null` on both GitHub connections (**verified** — GitHub OAuth tokens don't expire). The docs state the gateway auto-refreshes any provider token within five minutes of expiry (Slack et al.) (**assumed** — no expiring token was in scope). Under adoption, the planned RFC 9700 refresh worker would have shrunk to providers Connectors doesn't hold; with the route declined it keeps its original scope.
...
- **Consent flows:** two humans, two GitHub accounts, the two authorize URLs from §2. The consent screen shows Sprites' own GitHub OAuth app (client_id `Ov23lisx1GALDjNqHc2Z`) — branding is Fly's, not PageSpace's (product UX consideration, §6).
...
- The single-use `code` from flow A was still redeemable when completed at 16:53:58Z, after the human consent step (**verified** once; the consent timestamp was not recorded, so the code's expiry window was not measured).
...
Reading: the credential substitution happens inside the Sprites gateway after a Fly-signature identity check and a policy check on each of these calls (whether a label change is re-evaluated on every request or cached after first use is unverified — §4, §8 Q5). The two sprites talked to two *different* GitHub accounts; neither sprite could reach the other's credential; the guest never held, saw, or transmitted a token (nothing to hold — the API returns sanitized connections only, **verified** in every response). The 403 message names the missing labels — a mild information leak about policy shape to an unauthorized caller, acceptable inside an org.
...
- **Dynamic matching:** sprite D was created *after* conn A's policy existed, labeled via `PUT /v1/sprites/ps-s4-sprite-d-q2v8` with `{"labels":["ps-s4-label-a"]}`, and its first gateway call → **200**. No policy re-save, no propagation window observed. Label **removal** was not tested, so whether a later request is denied after a label is revoked (per-request evaluation vs cached first-use authorization) is unverified — §8 Q5.
...
- **Onprem:** Connectors are a Fly-hosted service; an onprem PageSpace deployment cannot use them. The Infisical plane (G1b-store, already merged) remains the onprem credential plane. Had Connectors been adopted, Infisical's scope would have shrunk but not died (it is still needed for `api_key`/`password`/`session` kinds against arbitrary origins).
- **Consent branding:** users consent to *Fly/Sprites'* GitHub OAuth app (client_id `Ov23lisx1GALDjNqHc2Z`, **verified** in the authorize_url), not a PageSpace-owned app. Under adoption this would have been a product decision: acceptable ("sprites connect to your GitHub") vs needing our own OAuth app (which Connectors does not support — the client is Sprites').

## 7. What adoption would have meant for the board — counterfactual

> **Not adopted.** This section records the consequences S4 projected *if* Connectors became the OAuth credential plane. D-34 declined the route; none of these edits were made, and the board runs the D-18 relay-first design (Λ1 full surface until the relay, G4 relay-only for REST + git, G2/G3/G1c original briefs). Read every item below as conditional.

1. **S1's operative conclusion is revised, not overturned.** S1: "no guest-originated trusted channel; G4 = server-side runner originates every credentialed op." The gateway is a *third* thing S1's taxonomy didn't contain: guest-originated **network** requests are unforgeable *at the gateway boundary*, because the substitution point is the Sprites edge itself. Under adoption, in-guest credentialed REST would have been safe **without** any broker grant/presenter key reaching the guest — the guest would only ever know a connection id and a URL. (This finding about the gateway is empirical and stands; the design consequence does not.)
2. **G4 would have become a split design:** REST credentialed ops → in-guest via gateway (connection ids resolved from PageSpace references at provision time); git transport → the planned server-side relay, unchanged, because the gateway cannot carry git. Λ1 (raw `GH_TOKEN` env var) would have been retired for REST + public-repo git, leaving only private-repo git use until the relay shipped.
3. **G2 (one custodian, scoped) would have kept its shape but changed its backing for OAuth:** the `oauth2` account kind would have become a thin reference row (provider, connection id, gateway base) + PageSpace-side permission semantics, with Sprites rather than Infisical as the credential plane for those rows. The api_key thin slice would have proceeded as planned (BYOK providers `openai`/`anthropic`/`openrouter`/… are *also* Connectors candidates — one `POST /v1/oauth/connections/api_key` could have replaced our stored-key path for those origins, leaving Infisical with arbitrary-origin custom APIs, passwords, sessions).
4. **G3 (migrate liabilities, refresh worker) would have shrunk hard:** GitHub connectors need no refresh (no expiry, **verified**); Slack-class expiry would have been the gateway's job (docs). The RFC 9700 worker would have remained only for providers neither Sprites nor we host.
5. **G1c (shape review) would have had to fold in:** the frozen `oauth2` shape mapped to (provider, connection_id, access_policy) with `allowed_endpoints`/`blocked_endpoints` as the origin+operation-pinning story; the no-mid-string-glob rule and exact-vs-prefix path semantics in the restriction grammar; approval (invariant 4) staying ours; a guard test banning mid-string glob patterns in anything we generate.
6. **Product/UX decisions adoption would have forced:** consent branding (Fly's app, §6); completion UX (default Sprites callback renders the code for our backend to complete — or ask Fly for a registered PageSpace `redirect_uri`); per-user isolation by **label discipline** (`ps-user-<hash>`), requiring sprite provisioning to set labels atomically and the permission layer to treat label→connection binding as a managed decision (invariant 5).

## 8. Open questions (moot unless the route is reopened)

1. Will Fly register a per-org OAuth callback (`redirect_uri`) so the completion code lands on PageSpace's backend directly instead of the copy-the-code page? (Not pursued after D-34; the copy-the-code flow works without it.)
2. Gateway rate limits / quotas per connection — undocumented; observed no limits in this spike's volume.
3. Scoped (non-unrestricted) Sprites tokens vs the Connectors CRUD API — docs say balance/usage endpoints 403 for scoped tokens; whether connection CRUD accepts scoped tokens is untested, so any reopening should assume the web-process token is full-authority (§6 blast radius).
...
5. Label revocation and policy-free connectors: does removing a sprite's label deny its next gateway call, and does a connection with no `access_policy` refuse every sprite? Neither was probed; both would need testing before relying on the policy as a revocable, fail-closed boundary.

## Appendix: raw evidence

Scratch scripts and token lived in the session scratchpad (`/tmp/opencode/s4/`, token 0600, deleted with the scratchpad). Sprites created: a-t5qk 16:55:23Z, b-w7n9, c-n4fr, d-q2v8; all deleted 17:03:53–17:04:17Z (24 s between the first and last deletion), `listAllSprites("ps-s4-")` → `[]`. Connections: conn A `kIVe_9LuZsRTBCx3M3xZDQ` inserted 2026-09-18T16:53:58.318Z, conn B `uWrelixHQ9op2uLNx4Xidw` inserted 16:53:59.160Z; both `DELETE` → 204, `GET /v1/oauth/connections` → `{"connections":[]}`. Key raw shapes:
