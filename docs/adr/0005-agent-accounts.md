...
| 5 | Exchange: `grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer`, `assertion=<secret>`, `client_id=pagespace-agent` — a new **static public client** whose allowed grants are jwt-bearer, the claim grant and `refresh_token`, with no redirect URIs. | Generic auth.md clients send the RFC 7523 URN. A separate client id keeps agent grants out of `pagespace-cli`'s allowed grants and separates audit and rate-limit keys. The registry edit is one additive object. |
...
| 13 | **Lifecycle** (**[D-28]**, recommended): an unclaimed agent that never authenticated after signup is deleted after 30 days; an agent that ever signed in has no TTL. | Abandoned signups are not entities yet; real agents are. |
...
- Whether an unclaimed agent can DM, invite or upload is decided by **[D-27]** through the single `emailVerified` stamp; no gate is edited either way.
- If [D-28] is answered differently, only `decideAgentLifecycle` and its test change.
