...
| `path` | dot-segments resolved. **A decoded copy is used for traversal and control-character checks only** (refuse if a `.`/`..` part or any control character appears once decoded); **the digested segment is normalized like a query component** — percent-encoding case normalized (upper-case hex), only the RFC 3986 unreserved set unescaped, **never decoded**. So `foo;bar` ≠ `foo%3Bbar`, `a+b` ≠ `a%2Bb`, `u@h` ≠ `u%40h`, `k=v` ≠ `k%3Dv`, and an encoded `/` (`%2F`) stays encoded and never becomes a segment boundary |
...
| `resources` | operation-specific identifiers the policy restricts on, as `[restrictionKey, value]` pairs sorted by key — **bound by the matched registry entry from the actual request**, never supplied by the caller: path slots of its `pathTemplate` (`/repos/{owner}/{repo}/…` binds `owner`, `repo`; slot matching is specified in full in the G1c amendment below), typed **body slots** read from the canonical JSON body (a Slack channel, mail recipients — R5), and for relay operations **derived resources** parsed from the git protocol request (ref names / branches of a `git-receive-pack` command list — R11), each emitted under the restriction key the entry maps it to. `[]` for `generic_request`. `CanonicalRequestInput` has no `resources` field, so a tool cannot claim repo A while the URL targets repo B (G1a review M8). Because the body and the derived refs are part of the digested request, the digest is recomputed over exactly those resources |
...
**Amendment 2026-09-16 (G1b review, under §3.3's invariant).** The first wording said the query was "canonically encoded" and projected `content-length` only when present; read as "decode, then hash", it let `?to=a+b` and `?to=a%2Bb` (a form-decoding server reads the first as `a b`), `?q=/safe` and `?q=%2Fsafe`, and `?x=a&b` and `?x=a%26b` share one digest, so approve-one/execute-another produced no `digest_mismatch`. An empty body with an explicit `content-length: 0` also digested differently from the identical request without it, and header values went through unchecked. The rows above now state the rule the G1b implementation (#2638) follows; no type changed. **Second amendment (same day, #2638 review):** the path row first said segments were "decoded once … re-encoded canonically", which is itself the defect for paths — decoding made `foo;bar`/`foo%3Bbar`, `+`/`%2B`, `@`/`%40` and `=`/`%3D` collide, because sub-delimiters are data-significant to the origin server. The path now follows the query rule for the digested bytes; decoding survives only as a check-only copy. Assertion §8.26 pins it, and its round-trip input must be built verbatim — a helper that re-encodes performs exactly the normalization under test.
...
**Amendment 2026-09-16 (G1c R7).** The registry is keyed by `(providerSlug, origin, channel, method, pathTemplate)`, and `OperationRegistryEntry.providerSlug` is never null: the first shape's null-provider entries matched every origin, and `providerSlug` — a mutable row field outside the plane's bindings — selected the catalogue. `providerSlug` is now inside `PlaneScope` (ADR 0005 §2.6), and an account whose `providerSlug` is null matches no entry at all (every request is `unknown`/`generic_request`).
...
### 4.3a Amendment 2026-09-16 (G1c R5, R12) — restrictions and delegation scope at issuance

`decideApproval` takes the account's `restrictions` (`ResourceRestrictions`, inside `PlaneScope`) and, for an unattended run, the `delegationScope` the delegation carries (`DelegationFact.scope`). Order: restrictions first — a restriction key the request does not bind is `out_of_scope` (an operation that cannot show which recipient it touches cannot be proven inside the list), and a bound value outside the list is `out_of_scope`; then the delegation scope — a request whose origin, operation or resources fall outside what the human delegated is `out_of_scope`; then the policy as in §4.3. The A2 promise (in-provider restriction by recipient/webhook target) is kept by typed body slots, not struck. `ApprovalScope.resources = []` restricts nothing; a key it names must be bound by the request.

### 4.4 Delegation and binding invalidation

- **Unattended run** (cron, workflow, a worker dispatched with no live session): the authority requires a `agent_account_delegations` row `(delegationId, accountId, agentPageId, delegatedBy, expiresAt, revokedAt, scope)` that is unexpired and unrevoked **and names this account, this agent page and this human** — the verifier's `DelegationFact` carries all four ids and F7 compares each against the signed grant, so a delegation for page P by user U never serves page Q or human V (G1a review H3); the grant carries `delegationId` and `human.sessionId = null`. No delegation → `no_delegation`. `DelegationFact` also carries the row's `scope` (`ApprovalScope`), and issuance passes it to `decideApproval`, so a delegation scoped to read-only repo A never authorizes everything the account policy allows (G1c R12).
- **Invalidation** is one mechanism: `policyVersion` bumps whenever the account's owner, tenant, origins, operations, approval policy, resource restrictions, bound agent page, or `sessionHttpEnabled` flag changes (turning `session_http` off must end outstanding `sessionHttp: true` grants, not let them run out their 15 minutes — G1a review M4); when the agent page is copied, moved across drives, or has its ownership transferred; when the bound agent's instructions or tool surface change (Λ15); and when drive membership of the owner changes. Every bump is written to the plane through `rebind` (ADR 0005 §2.2). Consent is required only when the new scope is not equal-or-narrower than the stored one, or the owner or pinned consenters change; a bump that only narrows or keeps the scope (membership change, instruction edit, owner deleted) needs none, so its causer need not be a consenter (G1c R13); a grant issued under the previous `policyVersion` is `policy_epoch` at the verifier and `bindings_stale` at the plane (G1c H2). Bindings are therefore invalidated, not re-evaluated by whoever views the conversation later.
...
**Amendment 2026-09-16 (G1c R6, R14).** `resourceIds` is no longer every declared key: after M8 every path slot was declared, so `/v1/tokens/{token}` put the token in the chain. `OperationRegistryEntry.auditResourceSlots` is an explicit allowlist, **empty by default**; `buildAuditRecord` takes the restriction keys of those slots (`auditResourceKeys`) and drops every other resource. A secret-bearing slot is simply never listed. `AuditOutcome.denied` carries `accountStatus` (the status behind `account_not_active`, withheld from the caller by F16, recorded here; null otherwise).
...
| F5a | `accountId` unknown (the adapter hands an `UnknownAccountBinding` — every account fact null, so an unknown account is representable and cannot claim a status or version; G1c R15; a drive-scoped caller still gets F3 `ceiling` first, exactly as for an account outside its drives, so scoped callers cannot tell unknown from out-of-scope), or `credentialVersion` ≠ `expected.currentCredentialVersion` — **except** `credentialVersion = expected.previousCredentialVersion` (plane-attested, non-null) with `now < rotatedAt + ROTATION_GRACE_MS` **and** `grant.iat < rotatedAt`, which is admitted (a grant in flight across a rotation; one issued after it never gets the old version). `revoke` clears the previous version, so a revoked account has no grace | `version_mismatch` |
| F6 | `policyVersion` ≠ current | `policy_epoch` |
| F7 | `delegationId` null while `human.sessionId` null; delegation expired/revoked; delegation fact whose `delegationId`, `accountId`, `agentPageId` or `delegatedBy` ≠ the grant's `delegationId`, `accountId`, `agentPageId` or `human.userId` | `no_delegation` (and at issuance, a request outside `DelegationFact.scope` is `out_of_scope`, §4.3a) |
...
//   G1c R9: generic on expected.aud; ok → VerifiedGrant<A>, branded with a unique symbol only verify-grant.ts attaches (no casts)
//   VerifyGrantInput<A>: { grant: unknown; signature: string; issuerPublicKey: Uint8Array;
//     now: number; expected: ExpectedBinding<A>;   // ExpectedRunBinding<A> & (KnownAccountBinding | UnknownAccountBinding) (R15)   // aud, presenter, CURRENT human/agentPageId/conversationId/runId,
...
export type DecideApproval = (input: { operation: OperationRef; restrictions: ResourceRestrictions;   // R5
  delegationScope: ApprovalScope | null;   // R12
  policy: AccountApprovalPolicy | null;
...
export type BuildAuditRecord = (input: { grant: AgentAccountGrant; canonical: CanonicalRequest;
  outcome: AuditOutcome; at: number; hash: HashBytes; auditResourceKeys: readonly string[] })   // R6 allowlist, empty by default
  => AgentAccountAuditRecord;   // never includes body, secret, header value, URL path or a resource outside the allowlist
```
...
26. Given query pairs `?to=a+b`/`?to=a%2Bb`, `?q=/safe`/`?q=%2Fsafe` and `?x=a&b`/`?x=a%26b`, `digestRequest` returns different digests for each pair; given lower-case escape hex or an escaped unreserved character, the same digest as the normalized form; given a body with and without a correct `content-length`, the same digest; given an admitted header value containing CR or LF, or a `content-length` that disagrees with the body, `canonicalizeRequest` refuses `malformed`; given PATH pairs `/x/foo;bar`/`/x/foo%3Bbar`, `/x/a+b`/`/x/a%2Bb`, `/x/u@h`/`/x/u%40h` and `/x/k=v`/`/x/k%3Dv`, different digests; given `/x/%2e%2e/y`, `path_traversal` (the check-only decoded copy). The §8.13 round-trip input is built verbatim, never re-encoded by the test (G1b review amendment; §3.2).
...
32. Given an account whose `sessionHttpEnabled` flips from `true` to `false` (or back), its `policyVersion` is bumped and written to the plane through `rebind`; an outstanding grant with `sessionHttp: true` issued before the flip is `policy_epoch` at the verifier and `bindings_stale` at the plane (G1c H2) (G1a review M4).
...
36. Given `lookupOperation` with an entry for `github`/`https://api.github.com:443` and a request to another origin, `null`; with `providerSlug: null`, `null` for every entry (G1c R7).
37. Given templates `/repos/{owner}/pulls` and `/repos/acme/pulls`, a request to `/repos/acme/pulls` matches the literal; `/repos/x/pulls` the slot; two entries identical in shape are a load-time conflict and `null` at lookup; `/repos/{owner}/{repo}/contents/{path+}` binds `path = 'a/b/c'`; `{path+}` not last is a registry defect; a path with an empty segment (`/repos//x`) matches nothing; a trailing-slash path does not match its slashless template; `restrictionKeys: { repo: 'github.repo' }` emits `['github.repo', 'b']` (G1c R17, table test).
38. Given a `chat.postMessage` entry with body slot `channel` and a JSON body `{"channel":"C1"}`, `canonical.resources` contains `['channel','C1']`; a body without `channel`, with a numeric `channel`, not JSON, with a duplicate or case-colliding key anywhere, sent without a JSON content type, or with a query argument named like a body slot, `malformed` — one reading of the request, whichever parser the provider uses (#2660 review); a `string_array` slot over `{"to":["a","b"]}` yields two pairs (G1c R5).
39. Given `decideApproval` with restrictions `{ channel: ['C1'] }` and a request that binds no `channel`, `refuse(out_of_scope)` whatever the policy; binding `C2`, `out_of_scope`; binding `C1`, falls through to the policy (G1c R5).
40. Given an unattended run whose `delegationScope` names only `read` on repo A, a `write` or repo B request is `refuse(out_of_scope)` even under an `always` policy that covers it (G1c R12).
41. Given `buildAuditRecord` for an entry whose `auditResourceSlots` is `[]` and a path `/v1/tokens/ghp_x`, `resourceIds` is `[]`; listing `repo` keeps only `repo` (G1c R6).
42. Given `deriveGitResources` over a `git-receive-pack` body updating `refs/heads/main` and `refs/tags/v1`, `receive_pack_ref_names` yields both and `receive_pack_branches` yields `main` and `refs/tags/v1` — every pushed ref gets a branch value, the full name for anything outside `refs/heads/`, so no branch allowlist admits a tag, `HEAD` or `refs/meta/*` pushed alongside an allowed branch (#2660 review); a ref named `-f` or containing NUL, `flag_injection`; truncated pkt-lines, `malformed`; and the relay canonical request's digest changes when a pushed ref changes (G1c R11).
43. Given `verifyGrant<'relay-runner'>`, the `ok` grant's type is `VerifiedGrant<'relay-runner'>` with no cast; an object literal with `__verified: true` is not assignable to `VerifiedGrant` (`@ts-expect-error`); `verified-grant-brand.ts` is imported by no production module but `verify-grant.ts` (G1c R9).
44. Given an `UnknownAccountBinding`, `version_mismatch` (not `account_not_active`); `AuditOutcome` `denied` for `account_not_active` records the status (G1c R14, R15).
