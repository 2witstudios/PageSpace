...
38. Given a `chat.postMessage` entry with body slot `channel` and a JSON body `{"channel":"C1"}`, `canonical.resources` contains `['channel','C1']`; a body without `channel`, with a numeric `channel`, or not JSON, `malformed`; a `string_array` slot over `{"to":["a","b"]}` yields two pairs (G1c R5).
...
42. Given `deriveGitResources` over a `git-receive-pack` body updating `refs/heads/main` and `refs/tags/v1`, `receive_pack_ref_names` yields both and `receive_pack_branches` yields `main`; a ref named `-f` or containing NUL, `flag_injection`; truncated pkt-lines, `malformed`; and the relay canonical request's digest changes when a pushed ref changes (G1c R11).
