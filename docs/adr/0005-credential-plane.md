...
5. Given stored bindings whose `ownerRef` or `allowedOrigins` differ (or whose `policyVersion` is newer — `bindings_stale`, §10.30) from those the grant's `bindingDigest` was computed over, `binding_mismatch`; `digestBindings` is key-order-independent (canonical JSON) so a reordered-but-equal copy matches.
