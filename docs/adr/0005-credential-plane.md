...
A row that still carries `pendingWrite` is **reconcile-required**: the Infisical write may have landed while the metadata commit failed. The next adapter call on that ref — any of put, rotate, resolve, rebind — takes the advisory lock, reads Infisical's current version and write digest, and hands them to `decideReconcile`: `commit_forward` (metadata advances to the pending version, grace opens only for a pending rotation) when they are exactly the pending write, otherwise `fail_closed` — nothing is served or written and the ref stays reconcile-required. **Amended 2026-09-21 (G2 ruling E1):** two exits keep a lost write from becoming a permanent outage. (a) A write the adapter knows it never sent (`not_sent`: the login failed, the request could not be built) clears its marker in the same locked section (`decidePendingOnWriteFailure`). (b) `decideReconcile` returns `abort_pending` when Infisical still shows the plane's committed `currentVersion` — every Infisical write creates a version, so the write provably did not land — and the marker is dropped. Anything else stays `fail_closed`. `resolve` never serves the ambiguous version: `decideResolve` refuses a reconcile-required ref with `store_unavailable`. `delete` (erasure) is not blocked by it. A **first put** has no row to carry an intent; if its commit failed, the orphaned Infisical key is handled by the next first put: `decideOrphanAdoption` adopts it only when its digest equals this put's attempted-write digest, otherwise the orphan is erased and the put proceeds. The write digest exists only while a write is ambiguous (cleared on commit) and never for a first put. **Amended 2026-09-21 (G2 ruling 4):** it is an HMAC-SHA3-256 under a plane-held `WriteDigestKey` (`AGENT_ACCOUNTS_WRITE_DIGEST_KEY`, ≥ 32 bytes, `parseWriteDigestKey`), never an unkeyed hash, so a reader of the metadata store cannot test candidate material against it. The pure decision `decideStoreWrite({ expectedVersion, observedBefore, observedAfter })` returns `commit | version_conflict | write_unverified` and is table-tested; the adapter only acts on it.
...
**Amendment 2026-09-16 (G1a review H2).** The first interface had no way to update `PlaneBindings.policyVersion` when it bumped: `put` needs the full material (the ingress identity is write-only and cannot read it back) and `rotate` is the refresh worker's alone. After any bump, either every grant failed with `binding_mismatch` or the authority signed stale bindings. `rebind` is the missing operation. `policyVersion` stays in the bindings: a non-widening bump (membership change, instruction edit, copy, move) still goes through `rebind` — consent-free when the scope is equal-or-narrower with owner and consenters unchanged (G1c R13), with a pinned consenter's consent otherwise — so outstanding grants die at the plane as `bindings_stale` (G1c H2; first written as `binding_mismatch`) as well as at the verifier as `policy_epoch`.

### 2.5 Metadata tables — which store holds which (corrected 2026-09-16, G1c R3)

**Main DB** (acceptable because none of it is secret and each row is compared as a fact against the signed grant, never trusted as plane state): `agent_account_grant_nonces` (ADR 0004 §2.4), `agent_account_approvals`, `agent_account_delegations`, `agent_account_audit` (ADR 0004 §5 shape). **Amended 2026-09-21 (G2 ruling 3):** the single-use ledger for `OwnerConsent.consentId` (E2) moved out of `agent_account_grant_nonces` into the plane store below — consumption is what stops a replay, and the main-DB writer is untrusted.

**Plane metadata store** (its own Postgres, never the app DB; one DDL owner, `store/infisical-dev/plane-metadata.sql`): `agent_account_secret_versions` (`ref`, `currentVersion`, `previousVersion`, `rotatedAt`, `revokedAt`, `revokeReason`, `pendingWrite`), `agent_account_plane_bindings` (`ref`, `bindings`, `scope`, `consenters`, CAS on `bindings.policyVersion`, §2.6) and `agent_account_consent_ledger` (`consentId` PK, `expiresAt`, `consumedAt`; G2 ruling 3). The first version of this section put `agent_account_secret_versions` in the main DB while M7 called `previousVersion`/`rotatedAt` plane-attested and `StoredSecretFacts.revokedAt` read from those rows: a main-DB writer could un-revoke a credential or reopen grace (`rotatedAt` far in the future). Those facts reach the verifier only through `describe` (`DescribeResult.previousVersion`, `rotatedAt`). The advisory-lock namespace is the plane's, distinct from `advisory-lock.ts`'s existing keys.
...
- **#2660 review follow-ups (same day).** The plane validates every `PlaneScope` value it compares (`isPlaneScopeWellFormed`): non-integer limits, a non-finite deadline or an unknown trigger compared false every way, so a widening passed as a narrowing; a malformed scope is never pinned, rebound to, or counted as narrowing. `rebind` refuses a revoked ref (`revoked`) without spending the consent. `rotate`'s grace snapshot must be the plane's version in Infisical, and `resolve` refuses a stored payload whose kind is not the ref's. Removing a policy (or an operation from it) counts as narrowing: it turns an `always` answer into a human prompt, never into more authority.
...
| F18 | `rebind`, `revoke`, `describe` or `delete` by any identity whose `channel` is not `manage`; `put` by any but `ingress`; `rotate` by any but `refresh-worker` | not representable by type, and `identity_refused` at runtime (`decideStoreCaller`, G1c E3 and the #2660 review). **Limit, stated (G2 ruling 2, 2026-09-21):** `channel` is asserted by the caller and every role shares one tenant identity (D-29 = B), so this is a **misuse guard** inside the plane's own code, **not a credential boundary** between roles: a holder of the tenant identity can act in every role for that tenant. It is a stated residual on Λ4 (threat model §3). Per-role identities are deliberately not added — they multiply the per-tenant identity count D-29 chose; a true boundary is a new decision revisiting D-29 |
...
5. Given stored bindings whose `ownerRef` or `allowedOrigins` differ from those the grant's `bindingDigest` was computed over, `binding_mismatch`; given stored bindings at a newer `policyVersion` than the grant's, `bindings_stale` (§10.30); `digestBindings` is key-order-independent (canonical JSON) so a reordered-but-equal copy matches.
...
## 10a. Amendment 2026-09-21 — L2·G2 implementation (what was built, and what it leaves open)

- **Topology.** The credential plane is its own process (`packages/lib/src/agent-accounts/executor/plane-worker.ts`, started by `bun run --filter @pagespace/lib credential-plane`, never exported — F13). It alone holds the per-tenant provisioner credential, the plane metadata DB, the `WriteDigestKey`, and (minted per operation, in memory) tenant identity client secrets. It verifies grants with the authority's PUBLIC key only. The web process holds the plane URL, the plane service secret, the presenter key id and the grant-signing key (§3.4) — nothing that reads material; the plane client has no read route. A source sweep of `apps/web/src` refuses any plane-side import or `INFISICAL_` variable (§10.11, now enforced).
- **D-29 = B at runtime.** One Infisical project and one `member`-role machine identity per tenant, created on first put under a per-tenant advisory lock (`infisical-tenant-provisioner-client.ts`). Only ids are recorded (`agent_account_plane_tenants`); client secrets are minted short-lived (600 s) per operation and never stored (§3.3 rider i). Stated residual: the provisioner credential can mint for any tenant — it is the plane's root, like Infisical's own root key, held as a plane process secret.
- **Web → plane.** Every call carries an HMAC over `t.METHOD.path.sha256(body)` under the shared service secret, valid 60 s (`plane-request-signature.ts`). It authenticates the caller and grants nothing: `put` is write-only, `revoke` only denies, `execute` still needs a signed grant. A captured request replayed inside 60 s is harmless for `revoke` and refused by CAS for `put`; `execute` is one-use by the grant nonce.
- **Executor.** Resolves once and pins to an all-public DNS answer, verifies TLS against the hostname, sends exactly the digested request plus `host` and the key, never follows a redirect, never decompresses, refuses upgrades, caps time/bytes/concurrency, and releases only `filterResponse`'s allowlisted, scrubbed view. An outcome the audit chain did not record is reported `outcome_unrecorded`, never success.
- **Left open by this slice (tracked on the task page):** policy use limits (`maxUsesPerHour`, `maxBytesOut`, `maxConcurrent`) are stored and signed but not yet counted — no usage ledger exists, so `limitsExceeded` is always false; delegations cannot be created, so unattended runs (workflows, cron) cannot use an account (`account_unavailable`); only the `api_key` kind and generic requests (no provider registry entries) are implemented; deployment packaging for the plane process (image, Fly app, onprem compose) is not built.
