...
25. *(Decision 6 — how an agent mints keys.)* `POST /api/auth/mcp-tokens` mints an `mcp_` key for a bearer caller only when `classifyCallerCredential` says the caller is an agent (`users.accountType = 'agent'`) presenting an `account`-scoped access token minted for `pagespace-agent` (its own jwt-bearer grant). The key is scoped by the same `validateDriveScopeAccess` rules as the session path, for that agent only, and limited by `AGENT_KEY_MINT` (10/hour per agent). Every other OAuth token (any human's, the agent's token held by another client, a narrow scope) keeps the route's 401 `OAuth tokens are not permitted for this endpoint`. *(Phase 2, mutation-checked; the key performs a content write in the real-Postgres door-flow test.)*
