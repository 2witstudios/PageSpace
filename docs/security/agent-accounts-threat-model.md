...
| A2 | Prompt injection → data exfiltration or harmful *authorized* action | provider scopes; in-provider resource restriction (repo/org/recipient/webhook target) over resources **extracted from the actual path** by the reviewed operation registry — the operation class and resources are never tool-declared (ADR 0004 §3.2, §3.4); approval bound to the digest; mediated egress; separate approval class for privilege expansion / recovery / token creation / bulk export | G2 (HTTP), G4 (relay), G6a (browser) | ASI01, ASI02 |
...
| A9 | Insider, DB dump or **DB tampering** | separate store/IAM/keys; plane-held bindings (owner, tenant, origins, policy version, policy digest over approval policy + resource restrictions + agent bindings) compared at resolve; delegation/approval rows checked as facts against the signed grant; independently authenticated owner consent for expansion; independent audit | G1b (bindings), G2 (consent step-up) | ASI03, ASI10 |
