...
| Account + credential version | `AccountId`, `CredentialVersion` | `agent_accounts.id` and the store's secret version | a grant for version *n* must not resolve version *n+1* after rotation; version *n* stays resolvable only inside the rotation grace and only for a grant issued before the rotation (ADR 0004 F5a) |
| Policy version | `PolicyVersion` | `agent_accounts.policyVersion`, bumped on every change to owner, origins, operations, approval policy, resource restrictions, bound agent pages or `sessionHttpEnabled` (ADR 0004 §4.4) | a grant issued under an older policy is refused after the owner tightens it |

**Store roles are not principals (G2 ruling 2, 2026-09-21).** `StoreIdentity.channel` names the role a plane caller acts in (`ingress`, `manage`, or one of the presenter channels), and the adapter refuses an operation whose required role the identity does not carry (ADR 0005 F18). That check is a **misuse guard, not a credential boundary**. Under D-29 every role in a tenant shares ONE Infisical machine identity, and the role is asserted by the calling code, so anyone who holds that tenant's identity — a compromised executor, ingress handler or management worker — can act in every role for that tenant. What bounds that compromise is the tenant scope of the identity (it reads no other tenant; Λ4) and the audit chain, not the role check. A true per-role boundary would need one identity per role per tenant, which multiplies the per-tenant identity count D-29 chose; revisiting that is a new decision, not an implementation change.
...
Main-DB integrity is a stated dependency: authorization reads mutable rows (owner, origins, policy). A main-DB writer who reassigns ownership is inside the threat model (attack row A9). Mitigation: the plane's own record independently binds `(tenantId, ownerRef, allowedOrigins, policyVersion, policyDigest, kind)` in the plane's own metadata store (never the main DB) — `policyDigest` is SHA3-256 over the approval policy, resource restrictions, bound agent pages, allowed and auxiliary origins, the `session_http` flag and the provider slug, so widening any of them without the plane's consent-gated `rebind` changes the digest even when `policyVersion` is left untouched — and the grant carries the authority's signed `bindingDigest` over the bindings it evaluated; the plane compares the two at resolve with no main-DB fact in the loop (ADR 0005 §2.4). Expansion of authority requires independently authenticated owner consent (step-up, `auth/step-up-decisions.ts`), carried to the plane as an authority-signed `OwnerConsent` over the exact new bindings and applied only by `rebind` under a manage-audience identity the web process never holds (ADR 0005 §2.2). A grant signed over tampered owner, tenant, kind, origin, policy, resource-restriction, agent-binding, `session_http` or provider rows therefore fails at the plane.

**Amendment 2026-09-16 (G1c, rulings under this section's invariants).** (1) *Who may consent is the plane's fact, not the main DB's*: the first put pins the consenters (`owner` for a user-owned account; the humans the authority read as drive OWNER/ADMIN for an agent-page-owned one); a writer who makes themselves ADMIN gets a real step-up and a real consent, and the plane refuses it because they are not pinned. Changing the set needs a current pinned consenter. An owner kind never changes. (2) *Consent names the account and is single-use* (consumed through the replay store). (3) *Rotation and revocation facts are plane-attested*: `agent_account_secret_versions` lives in the plane metadata store, so a writer cannot un-revoke or reopen grace. (4) *Narrowing needs no consent*, so a bump caused by someone who is not a consenter still reaches the plane; any widening does. (5) *An ambiguous write is never served*: a metadata commit failure after an Infisical write leaves the ref reconcile-required until a locked call proves the landed version is exactly the attempted write. **Residual:** while a replacing write is ambiguous the plane metadata store holds a SHA3-256 digest of the attempted value; a low-entropy `password` could be guessed offline from it by someone who reads that store during that window. The store is inside the plane's trust boundary (§7) and the digest is cleared on commit. **Delegation and approval rows are not in the plane's bindings**; they are protected by their own facts, each compared by the verifier against the signed grant: a delegation must name this account, this agent page and this human (ADR 0004 F7), an approval must name this account, this digest and this grant, and be unexpired at issuance (ADR 0004 F14). A writer who inserts a delegation or approval row still needs a grant the authority signs over it, and the audit chain records both.


**Amendment 2026-09-21 (G2, point-guard rulings on the G1c escalations).** (1) *The consent single-use ledger is the plane's.* `OwnerConsent.consentId` is consumed in the plane metadata store (`agent_account_consent_ledger`), not the main-DB nonce table: a main-DB writer could delete the consumed row and replay the consent. The `policyVersion` CAS on the bindings row stays as the second line. (2) *The pending-write digest is keyed.* It is an HMAC-SHA3-256 under a plane-held key that never enters the main DB or the web process, so the residual in the G1c amendment above no longer holds: a reader of the plane metadata store without that key cannot test candidate values against the digest. (3) *An ambiguous write is not a permanent outage.* A write the plane knows it never sent (login failure, unbuildable request) clears its own marker in the same locked section; otherwise the next locked call observes Infisical, and a copy still at the committed version proves the write did not land, so the marker is dropped and the account returns to service. **Residual:** a request that left the process, timed out, and reaches Infisical only after that observation lands unrecorded; the plane then refuses to serve it (the version no longer matches) and the account needs a fresh put to recover — an availability failure, never a wrong-material read.
...
| OAuth refresh token / client secret | the store; the **one** restricted refresh worker (ADR 0005 §5) | the refresh operation; no broker-wide cache | the HTTP executor and relay runner (they receive `OAuth2AccessMaterial`, which has no `refreshToken` field — ADR 0005 §4.2), Next.js, any second store (Λ5, D-24) |
...
| Session state (cookies, storage-state) | the store (kind `session`, declared format S3 §5); the isolated browser worker's ephemeral context; the HTTP executor only under the default-off `session_http` permission via `resolveSessionOverHttp` (ADR 0005 §4.2) | active browser session or single request; the persistent copy is store ciphertext | the agent Sprite, Sprite checkpoints (browser holder excluded from `checkpoint-policy.ts`), screenshots/a11y snapshots during login |
...
| A2 | Prompt injection → data exfiltration or harmful *authorized* action | provider scopes; in-provider resource restriction (repo/org/recipient/webhook target) over resources **extracted from the actual path, typed body slots and the git protocol request** by the reviewed operation registry — a restriction key the operation does not bind is refused `out_of_scope` (G1c R5/R11) — the operation class and resources are never tool-declared (ADR 0004 §3.2, §3.4); approval bound to the digest; mediated egress; separate approval class for privilege expansion / recovery / token creation / bulk export | G2 (HTTP), G4 (relay), G6a (browser) | ASI01, ASI02 |
| A3 | Confused deputy across users, agents, drives | authenticated identity intersection (§4); the verifier compares the grant's `human`/`agentPageId`/`conversationId`/`runId` with the presenter's *current* run (`principal_mismatch`, ADR 0004 F4a); a delegation serves only the account, agent page and human it names (`no_delegation`, F7); explicit `use` grants; immutable account binding (copies/moves/transfers invalidate bindings); live revocation | G1a/G1b define + verify; G2 enforce | ASI03 |
...
| A5 | Grant replay, incl. restored-sandbox replay | presenter binding; ≤ 15 min expiry; atomic nonce consumption in Postgres shared across replicas; `(spriteName, instanceId, generation)` binding (`binding_unavailable` when unobservable); account/policy epochs | G1b (replay store), G4 (generation case) | ASI03, ASI07 |
...
| A9 | Insider, DB dump or **DB tampering** | separate store/IAM/keys; plane-held bindings (owner, tenant, origins, policy version, policy digest over approval policy + resource restrictions + agent bindings + auxiliary origins + `session_http` + provider) compared at resolve; plane-pinned consenters and plane-attested rotation/revocation facts (§4 G1c amendment); delegation/approval rows checked as facts against the signed grant, the delegation's scope enforced at issuance; independently authenticated, ref-bound, single-use owner consent for expansion; independent audit whose resource values are an explicit allowlist | G1b (bindings), G2 (consent step-up) | ASI03, ASI10 |
...
| C4 | Credential rotated mid-grant | grant binds `credentialVersion`; resolve of a different version is `version_mismatch`, except the previous version inside the rotation grace for a grant issued before the rotation (ADR 0004 F5a, ADR 0005 F3) | G1b |
...
- **Λ2 — every integration/OAuth token sits in the main Postgres under one global `ENCRYPTION_KEY`** (nine decrypt sites, B0 §Λ2). *Open* → retired at L3. No user copy until L3 ships; the security claim is explicitly scoped until the last path moves.
  **Scoped claim while L3 is in progress (G3, 2026-09-23).** The §1 contract holds for accounts created through the G2 add-account path (`agent_accounts`, material in the plane only). It does **not** yet hold for `integration_connections.credentials`, `google_calendar_connections` or `zoom_connections`: those tokens are still ciphertext in the main Postgres under `ENCRYPTION_KEY`, decrypted in the web process. The exact set of places that still decrypt them is not prose: it is `LEGACY_PENDING_MIGRATION` in the decrypt guard's policy, enforced by a repo-wide test that fails on any new decryptor import outside the plane and on any list entry that no longer decrypts, so the list can only shrink. The claim widens as each entry leaves; Λ2 is retired when the list is empty and the legacy column is dropped. Two entries cannot leave inside G3 as briefed: the sandbox GitHub token (Λ1 needs the plaintext in the guest until the L4 relay, D-23) and the unattended Calendar/Zoom paths (sync, webhooks, disconnect run with no human or delegation the authority can sign a grant for). Both are escalated for a ruling.
  **Refresh-worker residuals (G3b, 2026-09-23).** Refresh is serialized per account (an in-process single flight plus a plane advisory try-lock with a re-resolve under it) and a rotation is persisted through the store's pending-write reconcile, so a crash between the Infisical write and the metadata commit keeps the rotated family. The worker refreshes only from the plane's current version: rotation grace would otherwise hand it a refresh token the provider already spent (found by the real-Infisical crash test; a reuse-detecting provider revokes the whole family on such a replay). **Stated residual:** a crash after the provider answered and before `rotate` recorded its intent loses the rotated refresh token; the next attempt presents the spent one, the provider answers `invalid_grant`, and the account is marked `needs_reauth` — an availability loss requiring the human to reconnect, never a wrong-material read.
...
- **Λ4 — Infisical is a trusted custodian.** Self-hosted Infisical OSS on its own app (D-21 revised). *Accepted* (D-21) → bounded by one project and one machine identity per tenant (**D-29** = B). **Stated residual (G2 ruling 2):** the plane's role check between ingress, executor and management callers is a misuse guard inside one tenant's identity, not a boundary between them (§3). **Copy (security page):** "Credentials are held by Infisical, a SOC 2 Type II secrets manager, under a key unique to your workspace. Infisical's operators, and the servers that use a credential on your behalf, are trusted; a breach there is a breach of your credentials and we will notify you under our breach runbook."
