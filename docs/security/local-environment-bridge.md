...
| Server policy editable by the owner only | `apps/web/src/app/api/drives/[driveId]/envs/[envId]/route.ts` (PATCH); `setServerPolicy` is a compare-and-set on `(envId, ownerId, revokedAt IS NULL)` in `drive-envs-store.ts` | **Yes.** |
...
| Revocation, both ways | `apps/web/src/lib/env-bridge/revoke.ts` and `packages/lib/src/services/drive-envs/local-env-revoke.ts` (stamp `revokedAt`, revoke every `env:bridge` session, signed `revoke` frame, close 1008); the daemon deletes its key on a verified revoke (`ws-client.ts` reducer effect `deleteKey`); Ctrl-C and `env disconnect` kill every process group the daemon started (`exec-runner.ts`, `killAll`) | **Yes.** DELETE on a local env revokes the machine first (`envs/[envId]/route.ts`). |
...
| R-8 | **Re-enrol after revoke.** A revoked env cannot take a new machine; the row must be deleted and recreated, which also discards the server policy and any approvals bookkeeping. | Low | Low (usability; no security loss) | Follow-up | page `m3cqduvg8pfl9zi7hn3okouf` |
