...
a challenge derived from the exact request the machine froze
(`packages/lib/src/env-bridge/owner-approval.ts`). **A server holding the signing key can no
