...
| **The human is proven to the machine** (hardening B) | `packages/lib/src/env-bridge/owner-approval.ts` (the challenge derived over `{domain, envId, challengeId, requestHash, scope}`, and the total verifier — COSE → JWK for every algorithm the app can register (ES256, EdDSA, RS256 — SimpleWebAuthn's default `supportedAlgorithmIDs`, pinned by a test against it), then `type` → challenge → origin → `rpIdHash` → user-present → credential in the pinned set → signature); `packages/cli/src/env-bridge/dispatcher.ts` (runs it before the byte-compare; anything else is `approval_unproven` and nothing runs); `packages/cli/src/env-bridge/crypto.ts` (`envBridgeSha256`, `es256Verify`); credentials pinned at enrolment in `packages/lib/src/services/drive-envs/drive-envs.ts` → `drive_env_local.ownerCredentials` and `packages/cli/src/credentials/serialize.ts` (`MachineHostCredential.ownerApproval`); relayed by `apps/web/src/app/api/env-bridge/approvals/[challengeId]/route.ts` and produced by `EnvApprovalCard.tsx` | **Yes.** Until this row existed the machine had NO proof a human clicked: the owner-only check lived entirely in the web route, which the daemon cannot observe, so anyone holding the signing key AND standing where the server stands could send a grant, read the challenge id and frozen request straight back out of the daemon's own `ask_pending` reply, and answer it themselves — the byte-compare passing by construction. Now a chat approval must carry a WebAuthn assertion from a credential pinned at enrolment, over a challenge derived from the request THIS machine froze, so it cannot be forged, moved to another request, another question, another machine, or — because the owner's chosen **scope** is in the binding too — relayed as a wider, durable approval than the one they granted. **Fail closed:** nothing pinned ⇒ the chat path is refused before a question is even frozen, and the terminal prompt (which never involved the server) is used instead. **Trust on first use is the boundary:** the set is written once, at enrolment, and no frame, no server answer and no CLI command may extend it — re-enrol to change it. |
...
`approvals/[challengeId]/route.ts`, `dispatcher.ts`).

**That click used to be SERVER-ATTESTED, and is not any more (hardening B).** Until this change
the daemon verified two things about an approval — the server's signature on the grant, and
that the re-issued request byte-matched the one it froze — and neither is evidence that a human
was there: the owner-only check ran in the web route, which the machine cannot observe, and the
daemon handed the challenge id and the frozen request back in its own `ask_pending` reply, so
the byte-compare passed by construction for anyone able to sign grants. The click is now proven
to the machine itself: it carries a WebAuthn assertion from a passkey pinned at enrolment, over
a challenge derived from the exact request the machine froze **and the scope the owner chose**
(`packages/lib/src/env-bridge/owner-approval.ts`) — so a proof given for "this once" cannot be
relayed as "until I revoke it", which would otherwise have let a compromised server turn a
single click into a standing capability. **A server holding the signing key can no
longer make a machine run anything a human did not approve.** Two things that remain true and
should not be over-read: the assertion proves *a human with the owner's authenticator pressed a
key for this request*, not that the human understood the request; and a compromised server still
chooses which frozen request the card *displays* beside the ceremony, so it can ask the owner to
approve one genuine pending request while showing the text of another — it can only ever get a
request the machine itself framed and the owner assented to, never one it invented. Three limits
of the click as a mitigation, stated plainly: (a) once a program is approved, a later steered command that runs only approved
...
**In practice the discovery list is LOCAL-ONLY.** A Sprite (cloud) environment has no enrolling
owner, so there is nobody the owner-only visibility write could be checked against and no
surface can set `visibleToGlobalAssistant` true for one. What ships is therefore *"any local
machine you made visible, plus this conversation's own sandbox"*, not *"any sandbox"*. That is
the fail-closed default and it is deliberate; nobody should read more into the feature than it
does.

Making cloud environments reachable is a separate decision, not a missing line of code. An env
is drive-owned, drive-paid and drive-shared, so its visibility would be decided by a drive
ROLE rather than by an enrolling owner — most plausibly the drive owner alone, since [D-6]
deliberately withholds Bind from drive admins for local machines and a cloud env should not be
laxer. Two things change if that is taken: `decideEnvReach` currently refuses an ownerless env
(`not_owner`) and would grow a second branch keyed on a drive role, and the discovery read's
ownership filter would need a membership filter beside it — which is exactly the cross-drive
aggregate trap PR #2609 hit. File it as its own decision.
...
| R-13 | **CLOSED (hardening B).** **The owner's click was attested by the server, not verified by the machine.** The daemon checked the grant signature and the byte-match and had no way to observe a human; the owner-only check lived in the web route. So an attacker holding the signing key AND the position of the server the daemon dials could drive the whole loop alone: send a grant → the daemon replies `ask_pending` and returns the challenge id and the frozen request → sign a second grant carrying `approvalIntent { challengeId, scope: 'until_revoked' }` → the byte-match passes by construction → it runs, and persists. Only in **chat-ask mode** — the headless path GA was built for; a TTY daemon mints no challenge, so a forged id died `unknown_challenge`. Nothing was ever exposed (`LOCAL_ENVS_ENABLED` off everywhere), so this was pre-release work. **Now:** the click carries a WebAuthn assertion from a passkey pinned at enrolment, over a challenge derived from the request the machine froze, verified by the machine before anything runs (`owner-approval.ts`, `dispatcher.ts`). | Was: requires signing-key compromise *and* server position | Was: high — arbitrary execution as the owner, persisting 30 days or forever | Closed | Leaves B1–B5, phase `lrk1o0ioddtexkad1id4d2te` |
| R-14 | **An owner with no passkey cannot use chat approvals at all.** The property has a price, and it is paid here: a machine that pinned nothing (or an empty set, because the owner had no passkey when they enrolled) refuses every chat approval and requires the terminal prompt, and there is deliberately no way to add a credential to a pinned set afterwards — re-enrolment is the only path, because a set anything could extend would be worth nothing. `env enroll`, `env connect` and the card each say so in one line. | Certain for an owner without a passkey | Medium: usability, not security — the fallback is strictly stronger, not weaker | Product | Documented here, in `packages/cli/README.md` and on the customer page |
| R-15 | **The card's text is still chosen by the server.** The assertion binds a click to one frozen request; it does not bind it to what the browser DISPLAYED. A compromised server could show the owner request A's text beside the ceremony for pending request B and obtain a valid assertion for B. It can only ever do this with a request the MACHINE itself framed and is holding — it cannot invent one — so the reachable set is bounded by what the owner's own agent asked for, which is exactly the surface R-6 already covers. Closing it properly needs the machine, not the server, to render what is signed. | Low (requires server compromise) | Medium, bounded by R-6's surface | Post-GA, with R-3 | Recorded here; no task yet |
...
- **Cloud (Sprite) environments in the global assistant's reach.** A Sprite env has no enrolling
  owner, so nothing can make one visible; the discovery list is local-only in practice. What
  would decide a cloud env's visibility is its own decision — see "What this is NOT, stated
  plainly" above.
