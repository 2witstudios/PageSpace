...
function parseArgs(argv: string[]): CliOptions {
  ...
  // `--dry-run --live` is contradictory; refuse rather than guess which won.
  if (opts.live && argv.includes('--dry-run')) {
 ...
 * fatal to the caller: an unrecorded successful send would be re-sent on the
 * next run, so we must never silently continue past it.
 */
...
function resolveBaseUrl(): string {
  const candidates = [process.env.NEXT_PUBLIC_APP_URL, process.env.WEB_APP_URL]
    .map((c) => c?.trim())
...
/**
 * Resolve and validate the marketing base URL for the docs links. MARKETING_BASE_URL
 * is operator-set; a malformed value (missing protocol, stray path, trailing slash)
 * would otherwise produce a broken link in a mass email. Returns the origin only.
 */
...
async function main(): Promise<void> {
  ...
  // enforced in code below. Rows come back encrypted (GDPR #965).
  const rows = await db
    .select({
      id: users.id,
      name: users.name,
