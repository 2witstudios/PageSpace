 ...
 *
 * Migration 0315 already runs this backfill on every deploy, in the same migrate
 * invocation as the walletId NOT NULL (0316) that depends on it — so there is never a
 * window where a ledger or hold row lacks a wallet. This script runs the SAME statements
 * (read from the 0315 file) in one transaction, for two jobs:
 *
 *   --dry-run   rehearse, then ROLL BACK. Writes nothing.
 *               - Database still at 0309 (production BEFORE the deploy): runs the whole
 *                 0310–0316 chain (org schema, then wallets) in one transaction and reports
 *                 the balance rows, the zero wallets 0315 would add, the rows it would assign, and any money
 ...
 *               - Database already through 0313: runs 0315 alone; against a fully
...
function describeRehearsal(r: WalletMigrationRehearsal): string {
  return [
    'DRY RUN from 0309 — migrations 0310–0316 applied in one transaction, then rolled back. Nothing written.',
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (atBase && !dryRun) {
      throw new Error('this database is still at 0309: the wallets table does not exist yet. Run the migrations (or --dry-run to rehearse them).');
    }
    if (atBase) {
